//! Namespace contexts and name resolution: building the file's [`NsCtx`] table
//! from its `namespace` / `use` declarations and resolving a [`NameRef`]
//! against the context enclosing it.

use mago_span::HasSpan;
use mago_syntax::cst::{Identifier, Node, Program, Statement, UseItems};

use crate::ast::{NameRef, NsCtx, RefKind};
use crate::{bytes_to_string, children, to_span};
...
pub(crate) fn name_ref(id: &Identifier<'_>) -> NameRef {
    let kind = match id {
...
pub(crate) fn build_contexts(program: &Program<'_>) -> (Vec<NsCtx>, Vec<(u32, u32, usize)>) {
...
pub(crate) fn use_binds_php_version_id(u: &mago_syntax::cst::Use<'_>) -> bool {
...
pub(crate) const PREG_FLAG_CONST_NAMES: &[&str] =
...
pub(crate) fn use_binds_preg_flag_const(u: &mago_syntax::cst::Use<'_>) -> bool {
...
pub(crate) fn ctx_of<'a>(contexts: &'a [NsCtx], regions: &[(u32, u32, usize)], offset: u32) -> &'a NsCtx {
...
pub(crate) fn fqn_of(ctx: &NsCtx, name: &str) -> String {
...
pub(crate) fn resolve_class_ref(ctx: &NsCtx, r: &NameRef) -> String {
    match r.kind {
...
pub(crate) struct RefResolver<'a> {
    pub(crate) contexts: &'a [NsCtx],
    pub(crate) regions: &'a [(u32, u32, usize)],
}
...
impl RefResolver<'_> {
    ...
    pub(crate) fn class_display_fqn(&self, r: &NameRef) -> String {

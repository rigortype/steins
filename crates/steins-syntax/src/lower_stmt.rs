//! Statement lowering: the [`Stmt`]/[`StmtKind`] trace entries, statement-end
//! reachability ([`BodyEnd`], ADR-0078), the `if`/`switch`/`match` shapes
...
pub(crate) fn lower_stmt(s: &Statement<'_>, out: &mut Vec<Stmt>) {
...
pub(crate) fn expr_is_true(expr: &Expression<'_>) -> bool {
...
pub(crate) fn expr_is_false(expr: &Expression<'_>) -> bool {
    match lower_arg_value(expr.unparenthesized()) {
        ArgValue::Bool(b) => !b,
        ArgValue::Int(i) => i == 0,
        ArgValue::Null => true,
...
pub(crate) fn scan_string_contexts(node: &Node<'_, '_>, out: &mut Vec<StringContextSite>) {
...
pub(crate) fn named_call(expr: &Expression<'_>) -> Option<CallExpr> {
    match expr.unparenthesized() {
...
pub(crate) fn call_invalidation(node: &Node<'_, '_>) -> Vec<InvalidatedVar> {
    let mut invalidated = Vec::new();
...
pub(crate) fn collect_direct_vars(node: &Node<'_, '_>, out: &mut Vec<String>) {
...
pub(crate) fn collect_foreach_sites(node: &Node<'_, '_>, scope_end: u32, out: &mut Vec<ForeachSite>) {
...
pub(crate) fn collect_operand_sites(node: &Node<'_, '_>, body: Option<Span>, out: &mut Vec<OperandSite>) {
...
pub(crate) fn collect_array_literal_sites(node: &Node<'_, '_>, out: &mut Vec<ArrayLiteralSite>) {
    match node {
...
/// [`Scope::poisoned`].
pub(crate) fn node_poisons(node: &Node<'_, '_>) -> bool {
...
pub(crate) fn push_byref_captures(

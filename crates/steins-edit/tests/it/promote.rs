...
//! (ADR-0034 / ADR-0037). Asserts on plan edits, refusal reasons, and rewrites.
...
fn zero_callers_refuses_no_observed_callers() {
    // ADR-0047 §4 / ADR-0041 §3: zero call sites is zero evidence — must refuse.
...
fn refuses_when_call_user_func_target_is_a_bare_variable() {
    // Issue #6: non-literal call_user_func target is invisible to the scan — taints broadly.
    let lib = "<?php\n/** @param int $x */\nfunction f($x) { return $x; }\n";
...
fn multibyte_source_is_promoted_correctly() {
    // Spans are byte offsets; the splice must stay on char boundaries.
    let lib =
...
fn refuses_string_default_for_int_param() {
    // `int $x = 'str'` is a compile-time fatal (wrong default type); must refuse.
...
fn refuses_float_default_for_int_param() {
    // Same fatal, float default (see above).
...
fn refuses_array_default_for_int_param() {
    // Same fatal, array default (see above).
    let lib = "<?php\n/** @param int $x */\nfunction f($x = []) { return $x; }\n";
    let report = plan(&[("lib.php", lib)]);
    assert_eq!(only_reason(&report), REASON_DEFAULT_INCOMPATIBLE);
    assert!(report.plan.is_empty());
...
fn refuses_unprovable_constant_default_conservatively() {
    // Legal PHP, but v1 can't prove the constant's type — refuse (sound, conservative).
...
fn promotes_compatible_int_default() {
    // Compatible default; a literal caller keeps this independent of the zero-caller gate.
...
fn promotes_nullable_with_null_default() {
    // `?int $x = null` is valid — nullable admits the null default (see above).
    let lib = "<?php\n/** @param int|null $x */\nfunction f($x = null) { return $x; }\n";
    let main = "<?php\nf(1);\n";
    let report = plan(&[("lib.php", lib), ("main.php", main)]);
    assert_eq!(report.oracle.transformed, 1, "{:#?}", report.refusals);
...
fn refuses_variadic_when_a_trailing_arg_is_bad() {
    // `f(1, 'str')` flows to variadic position 1 — every trailing arg must be proven.
...
/// An unproven `eval` anywhere refuses every candidate `eval-present`, recorded once.
#[test]
...
/// `eval('foo(42)')` calls `foo` with no CST call site — invisible to the
/// reference scan, so without the eval obstacle promotion would be unsound.
#[test]
...
/// A literal include resolving inside the universe (unlike below) is enumeration-benign.
#[test]
...
/// ADR-0046 vendor presumption: eval under `vendor/` is autoload plumbing, not an obstacle.
#[test]
...
/// A vouched eval site doesn't raise its obstacle; promotion proceeds but the
/// run downgrades its completeness claim (`vouched_exemptions`, ADR-0046 §2).
#[test]

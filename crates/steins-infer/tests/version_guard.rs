//! Issue #29 — `PHP_VERSION_ID` compared against an int literal folds against
//! the resolved target range (issue #28's), and the false arm dies through the
//! ORDINARY decided-branch pruning — `eval_cond` returns a verdict, `walk_if`
//! marks the skipped side dead, nothing version-specific downstream.
//!
//! The adversarial set the issue names is pinned here: inverted comparisons,
//! Yoda spellings, a straddling target (both arms stay live), and the
//! shadowing/aliasing corners where the fold must decline entirely.
...
/// is dead, and a proof-layer finding inside it disappears.
#[test]
fn an_old_php_branch_under_a_modern_floor_is_dead() {
    let src = "<?php\nif (PHP_VERSION_ID < 80000) { $x = null; $x->m(); }\n";
    assert_eq!(on_null_count(src, caret81()), 0);
    // The same guard with no target and no runtime stays live — no version, no
    // verdict, both arms walked.
...
/// Yoda spelling: the constant on the right mirrors the operator.
/// `80000 > PHP_VERSION_ID` asks `PHP_VERSION_ID < 80000`.
#[test]
...
/// Equality against a version outside the range is a definite `No`; inside the
/// range it stays `Maybe` (minor precision never pins one patch id).
#[test]
...
/// Liveness is branch-scoped: after the `if`'s tail, the join governs — a
/// decided guard never leaks its arm's env past the construct.
#[test]
fn the_tail_after_the_guard_sees_the_join() {
    // The guard is decided Yes (`>= 80100` under `^8.1`), so the then-arm's
    // assignment IS the fall-through env — and a later reassignment replaces
    // it. The dump after the construct must see the tail state, not a stale
    // branch snapshot.
...
fn a_userland_const_twin_disables_the_fold() {
    ...
    // Even the fully-qualified reference declines: the project-wide bit zeroes
    // the interval before per-reference reasoning starts. Conservative on
    // purpose; one weird file costs the feature, never a wrong branch.
...
/// import, not the engine constant — the fold declines in that file.
#[test]
...
/// A comparison against a non-int literal declines (the loose numeric-string
/// table is not modeled here).
#[test]

//! The declared-receiver lane (ADR-0049 §8 / S6), routed by minimum stratum (ADR-0049 A13).
//!
//! One ladder, two ids: the lane fires when a receiver's narrowed declared type provably
//! lacks the method, and its ladder adds descendant closure since a subclass — even one
//! minted by `eval` — could define it ("Conditional is not enough", §8). Which id carries
//! the result follows the arm's premise alone: a native declaration is `Verified`, so the
//! finding is `call.undefined-method` (proof layer); a `@param`/`@var` claim is `Asserted`,
//! so it stays `phpdoc.undefined-method` (contract layer).
//!
//! This file pins the **ladder**: each leg has a firing fixture and a silence fixture,
//! asserted over BOTH ids (a leg that only silenced the contract id would look green while
//! the promotion leaked past it). Routing, profile surfaces and copy-propagation widening
//! live in `s6_routing.rs`.
...
/// A boot-surface mock mirroring the S2 test harness: `available` is the A9 family-
/// availability gate, `builtins` the lowercased resident class-like names (A2ii homonyms),
/// `minor` the reported PHP `(major, minor)` override for the A11 skew leg.
struct Boot {
    available: bool,
    builtins: Vec<String>,
...
/// The proof-layer (all-`Verified`-premise) findings with a ready boot surface — the
/// A13-promoted half, told apart from S2's own emissions by the declared-receiver phrasing.
...
/// Every declared-receiver finding, whichever id A13 routed it to, under `folder` — the
/// silence legs assert over this, since a leg must silence the lane, not just move the id.
...
/// The conformance shape: `@param User|Guest`, `object` native, an `instanceof User` branch,
/// else-branch `$value->name()` narrowed to `{Guest}`; docblock arms are `Asserted`, so this
/// shape stays on the contract id.
...
fn fires_on_the_conformance_narrowing_shape() {
    ...
    // neither (contract-conditional, §8).
...
fn fires_on_a_single_final_declared_arm() {
    // Minimal shape: native `Guest` param (final, no `name()`), no narrowing needed. The
    // receiver is inexact (a param has no heap object) so this is the declared-receiver
    // lane's, not S2's, and the arm is native so A13 routes it to the proof-layer id.
...
fn fires_on_a_nonfinal_arm_with_a_fully_enumerated_clean_descendant_set() {
    // Base non-final: Sub is-a Base but adds no `tyop()`; universe resolvable, dam clear.
...
fn fires_on_a_native_union_narrowed_to_one_final_arm() {
    // Native `User|Guest`: negative `instanceof User` deletes User, leaving `{Guest}` (final,
    // no `pay()`) — still native, so still `Verified`.
...
fn fires_when_a_conditional_decl_in_chain_but_dam_is_clear() {
    // A2i: a conditional decl re-dams only when the dam isn't clear; no dynamism site here.
...
fn silent_when_a_descendant_defines_the_method() {
    // Base non-final; Sub (a Base) declares `tyop()` — runtime object may be a Sub. Silence.
...
fn silent_on_an_anonymous_class_extending_the_member() {
    // A4: an anonymous `extends Base` is invisible to the index — obstacle ⇒ silence.
...
fn silent_when_a_descendant_has_an_unresolvable_ancestor() {
    // Sub extends an external, uncatalogued class — whether it is-a Base is Unknown. Silence.
...
fn silent_when_the_dam_stands_via_a_conditional_decl() {
    // A2i + a dynamism site (`eval`) ⇒ the dam isn't clear; conditional Guest re-dams. Silence.
    let d = family("<?php\neval('$x = 1;');\nif (true) { final class Guest { public function guestId(): int { return 1; } } }\nfunction f(Guest $g): void { $g->name(); }\n");
    assert!(d.is_empty(), "{d:?}");
}
...
fn silent_when_the_dam_stands_for_a_nonfinal_arm() {
    // A non-final arm doesn't stop `eval` minting a subclass with the method. Silence.
    let d = family("<?php\neval('$x = 1;');\nclass Base {}\nfunction f(Base $b): void { $b->tyop(); }\n");
    assert!(d.is_empty(), "{d:?}");
}
...
fn silent_on_an_enum_arm() {
    // A3: enum bodies aren't lowered, so an enum arm reads method-empty. Unknown ⇒ silence.
...
fn silent_on_a_boot_surface_homonym() {
    // A2ii: the arm FQN is also a resident builtin, shadowed by the loaded class. Silence.
    let d = family_with(
        "<?php\nfinal class Guest { public function guestId(): int { return 1; } }\nfunction f(Guest $g): void { $g->name(); }\n",
...
fn silent_under_a_php_minor_skew_for_a_nonfinal_arm() {
    // A11: a PHP-minor skew from the catalog pin demotes closure to Unknown. Silence.
    let d = family_with(
        "<?php\nclass Base {}\nfunction f(Base $b): void { $b->tyop(); }\n",
...
fn silent_when_an_arm_is_a_scalar() {
    // `int|Guest` still carries a scalar arm — receiver may be a non-object. Silence.
...
fn silent_on_an_interface_arm_with_an_enum_implementor() {
    // A4: interface descendants include enum implementors; bodies are unlowered (A3). Silence.
...
fn descendant_via_a_class_alias_edge_is_detected() {
    // A4: parent matching follows a literal `class_alias` edge — a clean descendant reached
    // through it still fires; one that *defines* the method silences (genuinely detected,
    // not blanket-missed).
...
fn silent_when_an_ambiguous_fqn_descendant_half_defines_the_method() {
    // A4: enumeration is over *declarations*, so both Ambiguous-FQN halves count. Silence.
...
fn silent_on_a_lying_param_is_not_claimed_here_it_fires_correctly() {
    // A `@param User|Guest` whose runtime object is neither still fires under `{Guest}` —
    // CORRECT for the contract layer (§8: a declared-contract violation is real even if the
    // value lies). Not an FP; the lie can never reach the PROOF id (`s6_routing.rs`'s pin).
    let d = fires(CONFORMANCE_SHAPE);
...
fn exact_receiver_is_s2s_emitter_never_both() {
    // A13 makes S2 and the promoted declared-receiver half share ONE id, so disjointness is
    // over SITES not ids — each receiver is judged by exactly one emitter. `new Guest()` is
    // `class_exact` ⇒ S2's, and S2's evidence never carries the declared-receiver phrasing.

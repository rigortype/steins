...
//! What this file pins:
//!
//! * the headline — a native-param receiver's undefined method is a **default**
//!   surface finding, which is what the cross-check note's §3 probe measured as
//!   missing;
//! * the min-stratum rule at the boundary: one `Asserted` arm in a lane drags the
//!   whole finding back to the contract id;
//! * that no docblock claim — a lying `@param`, a lying `@phpstan-assert`, an
//!   inline `@var` — can forge the proof id;
//! * that the promoted path still stops at the A14 magic-tag obstacle, the
//!   ADR-0046 dam and the A9 sidecar gate;
//! * the piece-2 receiver-kind widening (a declared param copied to a variable) and
//!   the two shapes still deferred.
...
fn a_native_param_receiver_carries_the_proof_id() {
    // The cross-check note §3's `viaParam` probe: a bare `steins check` printed
    // nothing here, because the only id the lane could emit sat at the `Contracts`
    // floor. The arm is a native declaration — `Verified` — so A13 routes it to the
    // proof-layer id, with the declared-receiver evidence intact.
...
fn the_promoted_finding_is_on_the_default_surface() {
    ...
    // is special-cased for this lane.
...
fn a_lane_mixing_a_native_and_a_refined_arm_routes_asserted() {
    // `refine_contract_arms`: the `A` arm covers a native member exactly (`Verified`),
    // while `SubB` is a strict refinement inside the native `B` envelope (`Asserted`).
    // The minimum is `Asserted`, so the whole finding is contract-grade — one
    // unverified arm is enough, because the runtime receiver may be the arm the
    // docblock invented.
    let src = "<?php
final class A {}
class B {}
...
fn a_docblock_restating_the_native_type_stays_verified() {
    // The other side of the same rule: a `@param` that only repeats the native arm
    // adds no unverified premise (`arm_eq` ⇒ `Verified`), so the finding is still a
    // proof. A redundant docblock must not cost a user their default-surface finding.
    let src = "<?php
final class Guest { public function guestId(): int { return 1; } }
/** @param Guest $g */
...
fn a_lying_param_cannot_forge_the_proof_id() {
    ...
    // so the claim is `Asserted` and the finding is contract-grade — correct as a
    // contract violation (§8), never as a proof of a runtime break.
...
fn an_inline_var_cast_cannot_forge_the_proof_id() {
    ...
    // native envelope backs it), so a cast that narrows a receiver onto a
    // method-less class reaches the contract id and stops there.
    let src = "<?php
final class Guest { public function guestId(): int { return 1; } }
...
fn a_lying_phpstan_assert_cannot_forge_the_proof_id() {
    // The adversarial shape: without the tag the lane is `{Base}`, `Verified` and
    // SILENT (the descendant `Sub` answers `nope()`). A tag claiming the receiver is
    // the method-less `Guest` would, if it could mint a `Verified` class arm, turn
    // that silence into a proof-layer finding. It cannot — the assert lane narrows
    // facts at the `Asserted` stratum and never seeds a declared class arm — so the
    // shape stays silent on BOTH ids.
    let src = "<?php
final class Guest { public function guestId(): int { return 1; } }
class Base {}
...
fn the_promoted_path_still_stops_at_the_dam() {
    // ADR-0046: an `eval` site can mint a subclass carrying the method, so a
    // non-final arm's enumerated descendant set does not close. Native declaration
    // or not, the dam outranks the stratum.
    let clean = "<?php
class Base {}
...
fn the_promoted_path_still_needs_a_live_sidecar() {
    // A9: with the absence family unavailable (a monkey-patch extension, or no boot
    // surface at all) the lane is silent on both ids — promotion does not buy the
    // proof half an exemption.
...
fn a_declared_receiver_copied_to_a_variable_reaches_the_lane() {
    // `$c = $o;` binds the same value, so every declared possibility that held of
    // `$o` holds of `$c` at the same stratum. The copy carries the lane; the native
    // declaration keeps it `Verified`, so the copy's finding is a proof too.
    let src = "<?php
final class Guest { public function guestId(): int { return 1; } }
...
fn a_copy_narrowed_after_the_copy_still_closes() {
    // The copied lane is a live lane: a guard on the COPY subtracts from the copy's
    // arms (and only those), so the else-branch closes on the surviving native arm.
    let src = "<?php
...
fn a_return_typed_call_receiver_is_still_silent() {
    // Deferred with a reason: `mk()->m()` has no [`Receiver`] representation at all —
    // the trace lowers it to `Callee::Dynamic` — so admitting it is a syntax-crate
    // change (a new receiver variant plus every match on it), not a widening of this
    // lane's admission. Recorded, not smuggled in.
    let src = "<?php
final class Guest { public function guestId(): int { return 1; } }
...
fn one_call_site_is_judged_by_exactly_one_emitter() {
    ...
    // so the ADR-0049 §8 disjointness invariant is restated over SITES: an exact
    // receiver has no contract lane consulted (`is_exact` bails), and a lane-carrying
    // var is never `class_exact`. One finding per site, from one emitter.
    let src = "<?php
final class Guest { public function guestId(): int { return 1; } }

// @generated by `cargo xtask gen-catalog` from
// docs/research/phpstan-mining/declared_returns.toml — DO NOT EDIT BY HAND.
//
// Builtin DECLARED RETURN TYPES: the ADR-0069 Asserted floor
// (issues #73/#79, widened for the array vocabulary by ADR-0071).
// Without a live engine every rung of the return ladder is engine-gated and a
// builtin call with variable operands types as `unknown`; these rows raise that
// floor with the type the builtin DECLARES.
//
// LINEAGE — see the root NOTICE file for both MIT permission notices:
//   Steins <- phpstan-src `resources/functionMap.php`
//              (MIT, Copyright (c) Ondrej Mirtes and contributors)
//          <- Phan `src/Phan/Language/Internal/FunctionSignatureMap.php`
//              (MIT, Copyright (c) 2015 Rasmus Lerdorf,
//                   Copyright (c) 2015 Andrew Morrison)
//
// phpstan-src pin: dcde2be6ca3caae0e0d4bee6bfbc9fd39ea560e7
// cross-checked against PHP 8.5.8 via the real sidecar.
//
// Mining counts at the pin:
//   12461  functionMap entries (after the delta ladder)
//    6658  `Class::method` rows skipped (methods stay out of this slice)
//      18  names whose alternate signatures disagree on the return type
//    1352  rows the declared-contract arm lane cannot carry, of which
//       0    shaped arrays / lists
//     620    objects / class names / callable / resource
//      55  rows the arm-wise engine countersign refuses
//    2682  names the pinned engine does not know as functions
//    1606  ADMITTED (the table below), of which
//     687    RICHER than a single-base envelope (the #79 and ADR-0071 reach)
//
// The skipped methods and the object bucket are what remains deferred
// (ADR-0069 §5 as amended 2026-08-01, ADR-0071 §2.3). Object, class-name,
// `callable` and `resource` arms have no extensional denotation — the
// acceptance relation falls to a reflexive is-a floor and steins-contract
// carries no hierarchy — so the countersign could only answer `Maybe`, and a
// row entering uncountersigned is what ADR-0069 §3 refuses. The shaped-array
// bucket is empty since ADR-0071 gave the relation a structural denotation
// for `array` / `list<T>` / `array<K, V>` / `array{…}`.
//
// GRADE: every row seeds at the `Asserted` stratum, never `Verified`
// (ADR-0069 §2). It reaches the dump surface and contracts-tier reasoning;
// the proof layer's all-Verified premise rule keeps it out of every finding.
//
// WHEN IT SPEAKS: per NAME, not per run. The consuming rung fires exactly
// where the folder's reflected envelope yielded None for the asked name —
// `--no-php` is only the total case. With a live engine the floor still
// speaks where that engine is SILENT: an extension the analyzing PHP does
// not load, or a builtin with no declared return type. Where the engine
// answers, the floor never overrides it. The absence family never reads
// these rows at all: existence is a boot-surface fact, and an absence
// finding standing beside a floor fact is complementary, not contradictory.
//
// Each row: (lowercased builtin name, canonical phpdoc spelling — a base, a
// `T|false` failure union, a refinement, or an array type). The consumer
// re-lowers the string
// through the SAME `lower_str` → `flatten_arms` seam a PROJECT function's
// declared return takes (issue #60) and seeds the arms Asserted. Sorted by key
// for binary search.

pub(crate) static DECLARED_RETURNS: &[(&str, &str)] = &[
    ("_", "string"),
    ("abs", "positive-int|0|float"),
    ("acos", "float"),
    ("acosh", "float"),
    ("addcslashes", "string"),
    ("addslashes", "string"),
    ("array_change_key_case", "array"),
    ("array_chunk", "list<array>"),
    ("array_column", "array"),
    ("array_combine", "associative-array<mixed>"),
    ("array_count_values", "array<positive-int>"),
    ("array_diff", "array"),
    ("array_diff_assoc", "array"),
    ("array_diff_key", "array"),
    ("array_diff_uassoc", "array"),
    ("array_diff_ukey", "array"),
    ("array_fill", "array"),
    ("array_fill_keys", "array"),
    ("array_filter", "array"),
    ("array_flip", "array"),
    ("array_intersect", "array"),
    ("array_intersect_assoc", "array"),
    ("array_intersect_key", "array"),
    ("array_intersect_uassoc", "array"),
    ("array_intersect_ukey", "array"),
    ("array_key_exists", "bool"),
    ("array_key_first", "int|string|null"),
    ("array_key_last", "int|string|null"),
    ("array_keys", "list<int|string>"),
    ("array_map", "array"),
    ("array_merge", "array"),
    ("array_merge_recursive", "array"),
    ("array_multisort", "bool"),
    ("array_pad", "array"),
    ("array_product", "int|float"),
    ("array_push", "int"),
    ("array_replace", "array"),
    ("array_replace_recursive", "array"),
    ("array_reverse", "array"),
    ("array_search", "int|string|false"),
    ("array_slice", "array"),
    ("array_splice", "array"),
    ("array_sum", "int|float"),
    ("array_udiff", "array"),
    ("array_udiff_assoc", "array"),
    ("array_udiff_uassoc", "array"),
    ("array_uintersect", "array"),
    ("array_uintersect_assoc", "array"),
    ("array_uintersect_uassoc", "array"),
    ("array_unique", "array"),
    ("array_unshift", "positive-int"),
    ("array_values", "list<mixed>"),
    ("array_walk", "bool"),
    ("array_walk_recursive", "bool"),
    ("arsort", "bool"),
    ("asin", "float"),
    ("asinh", "float"),
    ("asort", "bool"),
    ("assert", "bool"),
    ("ast\\get_kind_name", "string"),
    ("ast\\get_metadata", "array<int, ast\\metadata>"),
    ("ast\\get_supported_versions", "array<int, int>"),
    ("ast\\kind_uses_flags", "bool"),
    ("atan", "float"),
    ("atan2", "float"),
    ("atanh", "float"),
    ("base64_encode", "string"),
    ("base_convert", "string"),
    ("basename", "string"),
    ("bcadd", "numeric-string"),
    ("bccomp", "0|1|-1"),
    ("bcdiv", "string"),
    ("bcmod", "string"),
    ("bcmul", "numeric-string"),
    ("bcpow", "numeric-string"),
    ("bcpowmod", "string"),
    ("bcround", "numeric-string"),
    ("bcscale", "int"),
    ("bcsqrt", "numeric-string"),
    ("bcsub", "numeric-string"),
    ("bin2hex", "string"),
    ("bind_textdomain_codeset", "string|false"),
    ("bindec", "int|float"),
    ("bindtextdomain", "string|false"),
    ("boolval", "bool"),
    ("bzclose", "bool"),
    ("bzcompress", "int|string"),
    ("bzdecompress", "int|string|false"),
    ("bzerrno", "int"),
    ("bzerror", "array"),
    ("bzerrstr", "string"),
    ("bzflush", "bool"),
    ("bzread", "string|false"),
    ("bzwrite", "int|false"),
    ("cal_days_in_month", "int"),
    ("cal_from_jd", "array"),
    ("cal_info", "array"),
    ("cal_to_jd", "int"),
    ("ceil", "float"),
    ("chdir", "bool"),
    ("checkdate", "bool"),
    ("checkdnsrr", "bool"),
    ("chgrp", "bool"),
    ("chmod", "bool"),
    ("chop", "string"),
    ("chown", "bool"),
    ("chr", "non-empty-string"),
    ("chunk_split", "string"),
    ("class_alias", "bool"),
    ("class_exists", "bool"),
    ("class_implements", "array<string,interface-string>|false"),
    ("class_parents", "array<string,class-string>|false"),
    ("class_uses", "array<string,trait-string>|false"),
    ("cli_set_process_title", "bool"),
    ("closelog", "bool"),
    ("collator_asort", "bool"),
    ("collator_compare", "int|false"),
    ("collator_get_attribute", "int|false"),
    ("collator_get_error_code", "int|false"),
    ("collator_get_error_message", "string|false"),
    ("collator_get_locale", "string|false"),
    ("collator_get_sort_key", "string|false"),
    ("collator_get_strength", "int"),
    ("collator_set_attribute", "bool"),
    ("collator_set_strength", "bool"),
    ("collator_sort", "bool"),
    ("collator_sort_with_sort_keys", "bool"),
    ("compact", "array<string, mixed>"),
    ("connection_aborted", "0|1"),
    ("convert_uudecode", "string|false"),
    ("convert_uuencode", "string"),
    ("copy", "bool"),
    ("cos", "float"),
    ("cosh", "float"),
    ("count", "positive-int|0"),
    ("count_chars", "string|array<int, int>"),
    ("crc32", "int"),
    ("crypt", "non-empty-string"),
    ("ctype_alnum", "bool"),
    ("ctype_alpha", "bool"),
    ("ctype_cntrl", "bool"),
    ("ctype_digit", "bool"),
    ("ctype_graph", "bool"),
    ("ctype_lower", "bool"),
    ("ctype_print", "bool"),
    ("ctype_punct", "bool"),
    ("ctype_space", "bool"),
    ("ctype_upper", "bool"),
    ("ctype_xdigit", "bool"),
    ("curl_errno", "int"),
    ("curl_error", "string"),
    ("curl_escape", "string|false"),
    ("curl_exec", "string|bool"),
    ("curl_multi_add_handle", "int"),
    ("curl_multi_errno", "int"),
    ("curl_multi_exec", "int"),
    ("curl_multi_getcontent", "string|null"),
    ("curl_multi_info_read", "false|array"),
    ("curl_multi_remove_handle", "int"),
    ("curl_multi_select", "int"),
    ("curl_multi_setopt", "bool"),
    ("curl_pause", "int"),
    ("curl_setopt", "bool"),
    ("curl_setopt_array", "bool"),
    ("curl_share_errno", "int"),
    ("curl_share_setopt", "bool"),
    ("curl_unescape", "string|false"),
    ("curl_version", "false|array"),
    ("date", "string"),
    ("date_default_timezone_get", "string"),
    ("date_default_timezone_set", "bool"),
    ("date_format", "string"),
    ("date_get_last_errors", "false|array{error_count: positive-int|0, errors: list<string>, warning_count: positive-int|0, warnings: list<string>}"),
    ("date_interval_format", "string"),
    ("date_offset_get", "int"),
    ("date_parse", "array<string, mixed>"),
    ("date_parse_from_format", "array{day: int|false, error_count: int, errors: array<string>, fraction: float|false, hour: int|false, is_dst?: bool, is_localtime: bool, minute: int|false, month: int|false, relative?: array{day: int, first_day_of_month?: bool, hour: int, last_day_of_month?: bool, minute: int, month: int, second: int, weekday?: int, weekdays?: int, year: int}, second: int|false, tz_abbr?: string, tz_id?: string, warning_count: int, warnings: array<string>, year: int|false, zone?: int|bool, zone_type?: int|bool}"),
    ("date_sun_info", "array{astronomical_twilight_begin: int|bool, astronomical_twilight_end: int|bool, civil_twilight_begin: int|bool, civil_twilight_end: int|bool, nautical_twilight_begin: int|bool, nautical_twilight_end: int|bool, sunrise: int|bool, sunset: int|bool, transit: int|bool}"),
    ("date_timestamp_get", "int"),
    ("datefmt_format", "string|false"),
    ("datefmt_format_object", "string|false"),
    ("datefmt_get_calendar", "int|false"),
    ("datefmt_get_datetype", "int|false"),
    ("datefmt_get_error_code", "int"),
    ("datefmt_get_error_message", "string"),
    ("datefmt_get_locale", "string|false"),
    ("datefmt_get_pattern", "string|false"),
    ("datefmt_get_timetype", "int|false"),
    ("datefmt_get_timezone_id", "string|false"),
    ("datefmt_is_lenient", "bool"),
    ("datefmt_localtime", "false|array"),
    ("datefmt_parse", "int|float|false"),
    ("datefmt_set_calendar", "bool"),
    ("datefmt_set_pattern", "bool"),
    ("datefmt_set_timezone", "bool"),
    ("dba_delete", "bool"),
    ("dba_exists", "bool"),
    ("dba_fetch", "string|false"),
    ("dba_firstkey", "string|false"),
    ("dba_handlers", "array"),
    ("dba_insert", "bool"),
    ("dba_key_split", "false|array"),
    ("dba_list", "array"),
    ("dba_nextkey", "string|false"),
    ("dba_optimize", "bool"),
    ("dba_replace", "bool"),
    ("dba_sync", "bool"),
    ("dcgettext", "string"),
    ("dcngettext", "string"),
    ("debug_backtrace", "list<array{function:string,line?:int,file?:string,class?:class-string,type?:'::'|'->',args?:list<mixed>,object?:object}>"),
    ("decbin", "string"),
    ("dechex", "string"),
    ("decoct", "string"),
    ("define", "bool"),
    ("defined", "bool"),
    ("deflate_add", "string|false"),
    ("deg2rad", "float"),
    ("dgettext", "string"),
    ("dirname", "string"),
    ("disk_free_space", "float|false"),
    ("disk_total_space", "float|false"),
    ("diskfreespace", "float|false"),
    ("dl", "bool"),
    ("dngettext", "string"),
    ("dns_check_record", "bool"),
    ("dns_get_mx", "bool"),
    ("dns_get_record", "false|list<array>"),
    ("doubleval", "float"),
    ("easter_date", "int"),
    ("easter_days", "int"),
    ("error_get_last", "null|array{file: string, line: int, message: string, type: int}"),
    ("error_log", "bool"),
    ("error_reporting", "int"),
    ("escapeshellarg", "string"),
    ("escapeshellcmd", "string"),
    ("exec", "string|false"),
    ("exif_imagetype", "int|false"),
    ("exif_read_data", "false|array"),
    ("exif_tagname", "string|false"),
    ("exif_thumbnail", "string|false"),
    ("exp", "float"),
    ("explode", "list<string>"),
    ("expm1", "float"),
    ("extension_loaded", "bool"),
    ("extract", "positive-int|0"),
    ("fclose", "bool"),
    ("fdiv", "float"),
    ("feof", "bool"),
    ("fflush", "bool"),
    ("fgetc", "string|false"),
    ("fgetcsv", "false|non-empty-list<string>|list{null}"),
    ("fgets", "string|false"),
    ("file", "false|list<string>"),
    ("file_exists", "bool"),
    ("file_get_contents", "string|false"),
    ("file_put_contents", "positive-int|0|false"),
    ("fileatime", "int|false"),
    ("filectime", "int|false"),
    ("filegroup", "int|false"),
    ("fileinode", "int|false"),
    ("filemtime", "int|false"),
    ("fileowner", "int|false"),
    ("fileperms", "int|false"),
    ("filesize", "positive-int|0|false"),
    ("filetype", "string|false"),
    ("filter_has_var", "bool"),
    ("filter_id", "int|false"),
    ("filter_input_array", "false|null|array"),
    ("filter_list", "non-empty-list<non-falsy-string>"),
    ("filter_var_array", "false|null|array"),
    ("finfo_buffer", "string|false"),
    ("finfo_close", "bool"),
    ("finfo_file", "string|false"),
    ("finfo_set_flags", "bool"),
    ("floatval", "float"),
    ("flock", "bool"),
    ("floor", "float"),
    ("fmod", "float"),
    ("fnmatch", "bool"),
    ("fprintf", "int"),
    ("fputcsv", "positive-int|0|false"),
    ("fputs", "positive-int|0|false"),
    ("fread", "string|false"),
    ("frenchtojd", "int"),
    ("fseek", "0|-1"),
    ("fstat", "false|array"),
    ("ftell", "int|false"),
    ("ftok", "int"),
    ("ftp_alloc", "bool"),
    ("ftp_append", "bool"),
    ("ftp_cdup", "bool"),
    ("ftp_chdir", "bool"),
    ("ftp_chmod", "int|false"),
    ("ftp_close", "bool"),
    ("ftp_delete", "bool"),
    ("ftp_exec", "bool"),
    ("ftp_fget", "bool"),
    ("ftp_fput", "bool"),
    ("ftp_get", "bool"),
    ("ftp_login", "bool"),
    ("ftp_mdtm", "int"),
    ("ftp_mkdir", "string|false"),
    ("ftp_mlsd", "false|array"),
    ("ftp_nb_continue", "int"),
    ("ftp_nb_fget", "int"),
    ("ftp_nb_fput", "int"),
    ("ftp_nb_get", "int|false"),
    ("ftp_nb_put", "int|false"),
    ("ftp_nlist", "false|array"),
    ("ftp_pasv", "bool"),
    ("ftp_put", "bool"),
    ("ftp_pwd", "string|false"),
    ("ftp_rawlist", "false|array"),
    ("ftp_rename", "bool"),
    ("ftp_rmdir", "bool"),
    ("ftp_set_option", "bool"),
    ("ftp_site", "bool"),
    ("ftp_size", "int"),
    ("ftp_systype", "string|false"),
    ("ftruncate", "bool"),
    ("func_get_args", "list<mixed>"),
    ("func_num_args", "positive-int|0"),
    ("function_exists", "bool"),
    ("fwrite", "positive-int|0|false"),
    ("gc_collect_cycles", "int"),
    ("gc_enabled", "bool"),
    ("gc_mem_caches", "int"),
    ("gc_status", "array{application_time: float, buffer_size: int, collected: int, collector_time: float, destructor_time: float, free_time: float, full: bool, protected: bool, roots: int, running: bool, runs: int, threshold: int}"),
    ("gd_info", "array"),
    ("get_class_methods", "list<non-falsy-string>"),
    ("get_class_vars", "array"),
    ("get_current_user", "string"),
    ("get_debug_type", "string"),
    ("get_declared_classes", "list<class-string>"),
    ("get_declared_interfaces", "list<interface-string>"),
    ("get_declared_traits", "list<trait-string>"),
    ("get_defined_constants", "array<string, mixed>"),
    ("get_defined_functions", "array{internal:non-empty-list<callable-string>,user:list<callable-string>}"),
    ("get_defined_vars", "array<string, mixed>"),
    ("get_extension_funcs", "list<callable-string>|false"),
    ("get_headers", "false|array"),
    ("get_html_translation_table", "array"),
    ("get_include_path", "string|false"),
    ("get_included_files", "list<string>"),
    ("get_loaded_extensions", "list<string>"),
    ("get_mangled_object_vars", "array"),
    ("get_meta_tags", "false|array"),
    ("get_object_vars", "array<mixed>"),
    ("get_required_files", "list<string>"),
    ("get_resource_id", "int"),
    ("get_resource_type", "string"),
    ("get_resources", "array<int, resource>"),
    ("getcwd", "non-empty-string|false"),
    ("getdate", "array{seconds: int<0, 59>, minutes: int<0, 59>, hours: int<0, 23>, mday: int<1, 31>, wday: int<0, 6>, mon: int<1, 12>, year: int, yday: int<0, 365>, weekday: \"Monday\"|\"Tuesday\"|\"Wednesday\"|\"Thursday\"|\"Friday\"|\"Saturday\"|\"Sunday\", month: \"January\"|\"February\"|\"March\"|\"April\"|\"May\"|\"June\"|\"July\"|\"August\"|\"September\"|\"October\"|\"November\"|\"December\", 0: int}"),
    ("gethostbyaddr", "string|false"),
    ("gethostbyname", "string"),
    ("gethostbynamel", "false|list<string>"),
    ("gethostname", "string|false"),
    ("getimagesize", "false|array{positive-int|0, positive-int|0, int, string, bits?: int, channels?: int, mime: string}"),
    ("getimagesizefromstring", "false|array{positive-int|0, positive-int|0, int, string, bits?: int, channels?: int, mime: string}"),
    ("getlastmod", "int|false"),
    ("getmxrr", "bool"),
    ("getmygid", "int|false"),
    ("getmyinode", "int|false"),
    ("getmypid", "int|false"),
    ("getmyuid", "int|false"),
    ("getopt", "false|array<string, string>|array<string, false>|array<string, list<mixed>>"),
    ("getprotobyname", "int|false"),
    ("getprotobynumber", "string|false"),
    ("getrandmax", "int"),
    ("getrusage", "false|array"),
    ("getservbyname", "int|false"),
    ("getservbyport", "string|false"),
    ("gettext", "string"),
    ("gettimeofday", "float|array"),
    ("gettype", "string"),
    ("glob", "false|list<string>"),
    ("gmdate", "string"),
    ("gmmktime", "int|false"),
    ("gmp_cmp", "int"),
    ("gmp_div_qr", "array"),
    ("gmp_export", "string"),
    ("gmp_gcdext", "array"),
    ("gmp_hamdist", "int"),
    ("gmp_intval", "int"),
    ("gmp_jacobi", "int"),
    ("gmp_kronecker", "int"),
    ("gmp_legendre", "int"),
    ("gmp_perfect_power", "bool"),
    ("gmp_perfect_square", "bool"),
    ("gmp_popcount", "int"),
    ("gmp_prob_prime", "int"),
    ("gmp_rootrem", "array<int, gmp>"),
    ("gmp_scan0", "int"),
    ("gmp_scan1", "int"),
    ("gmp_sign", "int"),
    ("gmp_sqrtrem", "array"),
    ("gmp_strval", "string"),
    ("gmp_testbit", "bool"),
    ("gmstrftime", "string|false"),
    ("gnupg_adddecryptkey", "bool"),
    ("gnupg_addencryptkey", "bool"),
    ("gnupg_addsignkey", "bool"),
    ("gnupg_cleardecryptkeys", "bool"),
    ("gnupg_clearencryptkeys", "bool"),
    ("gnupg_clearsignkeys", "bool"),
    ("gnupg_decrypt", "string|false"),
    ("gnupg_decryptverify", "array"),
    ("gnupg_deletekey", "bool"),
    ("gnupg_encrypt", "string|false"),
    ("gnupg_encryptsign", "string|false"),
    ("gnupg_export", "string|false"),
    ("gnupg_getengineinfo", "array"),
    ("gnupg_geterror", "string|false"),
    ("gnupg_getprotocol", "int"),
    ("gnupg_import", "false|array"),
    ("gnupg_keyinfo", "false|array"),
    ("gnupg_listsignatures", "null|array"),
    ("gnupg_setarmor", "bool"),
    ("gnupg_setsignmode", "bool"),
    ("gnupg_sign", "string|false"),
    ("gnupg_verify", "false|array"),
    ("grapheme_extract", "string|false"),
    ("grapheme_stripos", "int|false"),
    ("grapheme_stristr", "string|false"),
    ("grapheme_strpos", "int|false"),
    ("grapheme_strripos", "int|false"),
    ("grapheme_strrpos", "int|false"),
    ("grapheme_strstr", "string|false"),
    ("grapheme_substr", "string|false"),
    ("gregoriantojd", "int"),
    ("gzclose", "bool"),
    ("gzcompress", "string|false"),
    ("gzdecode", "string|false"),
    ("gzdeflate", "string|false"),
    ("gzencode", "string|false"),
    ("gzeof", "bool"),
    ("gzfile", "false|list<string>"),
    ("gzgetc", "string|false"),
    ("gzgets", "string|false"),
    ("gzinflate", "string|false"),
    ("gzpassthru", "int|false"),
    ("gzputs", "int|false"),
    ("gzread", "string|false"),
    ("gzrewind", "bool"),
    ("gzseek", "int"),
    ("gztell", "int|false"),
    ("gzuncompress", "string|false"),
    ("gzwrite", "int|false"),
    ("hash", "non-falsy-string"),
    ("hash_algos", "non-empty-list<non-falsy-string>"),
    ("hash_equals", "bool"),
    ("hash_file", "non-falsy-string|false"),
    ("hash_final", "non-falsy-string"),
    ("hash_hkdf", "non-falsy-string"),
    ("hash_hmac", "non-falsy-string"),
    ("hash_hmac_algos", "non-empty-list<non-falsy-string>"),
    ("hash_hmac_file", "non-falsy-string|false"),
    ("hash_pbkdf2", "non-falsy-string"),
    ("hash_update", "bool"),
    ("hash_update_file", "bool"),
    ("hash_update_stream", "int"),
    ("header_register_callback", "bool"),
    ("headers_list", "list<non-empty-string>"),
    ("headers_sent", "bool"),
    ("hebrev", "string"),
    ("hex2bin", "string|false"),
    ("hexdec", "int|float"),
    ("highlight_file", "string|bool"),
    ("highlight_string", "string|bool"),
    ("hrtime", "int|float|false|list{int, int}"),
    ("html_entity_decode", "string"),
    ("htmlentities", "string"),
    ("htmlspecialchars", "string"),
    ("htmlspecialchars_decode", "string"),
    ("http_build_query", "string"),
    ("http_get_last_response_headers", "null|list<string>"),
    ("http_response_code", "int|bool"),
    ("hypot", "float"),
    ("iconv", "string|false"),
    ("iconv_mime_decode", "string|false"),
    ("iconv_mime_decode_headers", "false|array"),
    ("iconv_mime_encode", "string|false"),
    ("iconv_set_encoding", "bool"),
    ("iconv_strlen", "positive-int|0|false"),
    ("iconv_strpos", "int|false"),
    ("iconv_strrpos", "int|false"),
    ("iconv_substr", "string|false"),
    ("idate", "int|false"),
    ("idn_to_ascii", "string|false"),
    ("idn_to_utf8", "string|false"),
    ("ignore_user_abort", "0|1"),
    ("image_type_to_extension", "string|false"),
    ("image_type_to_mime_type", "string"),
    ("imageaffinematrixconcat", "false|list{float, float, float, float, float, float}"),
    ("imageaffinematrixget", "false|list{float, float, float, float, float, float}"),
    ("imagealphablending", "bool"),
    ("imageantialias", "bool"),
    ("imagearc", "bool"),
    ("imagebmp", "bool"),
    ("imagechar", "bool"),
    ("imagecharup", "bool"),
    ("imagecolorallocate", "non-negative-int|false"),
    ("imagecolorallocatealpha", "non-negative-int|false"),
    ("imagecolorat", "non-negative-int|false"),
    ("imagecolorclosest", "non-negative-int"),
    ("imagecolorclosestalpha", "non-negative-int"),
    ("imagecolorclosesthwb", "non-negative-int"),
    ("imagecolordeallocate", "bool"),
    ("imagecolormatch", "bool"),
    ("imagecolorresolve", "non-negative-int"),
    ("imagecolorresolvealpha", "non-negative-int"),
    ("imagecolorsforindex", "array{alpha: int<0, 127>, blue: int<0, 255>, green: int<0, 255>, red: int<0, 255>}"),
    ("imagecolorstotal", "int<0, 256>"),
    ("imagecolortransparent", "int"),
    ("imageconvolution", "bool"),
    ("imagecopy", "bool"),
    ("imagecopymerge", "bool"),
    ("imagecopymergegray", "bool"),
    ("imagecopyresampled", "bool"),
    ("imagecopyresized", "bool"),
    ("imagedashedline", "bool"),
    ("imagedestroy", "bool"),
    ("imageellipse", "bool"),
    ("imagefill", "bool"),
    ("imagefilledarc", "bool"),
    ("imagefilledellipse", "bool"),
    ("imagefilledpolygon", "bool"),
    ("imagefilledrectangle", "bool"),
    ("imagefilltoborder", "bool"),
    ("imagefilter", "bool"),
    ("imageflip", "bool"),
    ("imagefontheight", "int"),
    ("imagefontwidth", "int"),
    ("imageftbbox", "false|array"),
    ("imagefttext", "false|array"),
    ("imagegammacorrect", "bool"),
    ("imagegd", "bool"),
    ("imagegd2", "bool"),
    ("imagegetclip", "array<int, int>"),
    ("imagegif", "bool"),
    ("imageistruecolor", "bool"),
    ("imagejpeg", "bool"),
    ("imagelayereffect", "bool"),
    ("imageline", "bool"),
    ("imageopenpolygon", "bool"),
    ("imagepalettetotruecolor", "bool"),
    ("imagepng", "bool"),
    ("imagepolygon", "bool"),
    ("imagerectangle", "bool"),
    ("imagesavealpha", "bool"),
    ("imagesetbrush", "bool"),
    ("imagesetclip", "bool"),
    ("imagesetinterpolation", "bool"),
    ("imagesetpixel", "bool"),
    ("imagesetstyle", "bool"),
    ("imagesetthickness", "bool"),
    ("imagesettile", "bool"),
    ("imagestring", "bool"),
    ("imagestringup", "bool"),
    ("imagesx", "positive-int"),
    ("imagesy", "positive-int"),
    ("imagetruecolortopalette", "bool"),
    ("imagettfbbox", "false|array"),
    ("imagettftext", "false|array"),
    ("imagetypes", "int"),
    ("imagewbmp", "bool"),
    ("imagewebp", "bool"),
    ("imagexbm", "bool"),
    ("implode", "string"),
    ("in_array", "bool"),
    ("inet_ntop", "string|false"),
    ("inet_pton", "string|false"),
    ("inflate_add", "string|false"),
    ("inflate_get_read_len", "int|false"),
    ("inflate_get_status", "int|false"),
    ("ini_alter", "string|false"),
    ("ini_get", "string|false"),
    ("ini_get_all", "false|array"),
    ("ini_set", "string|false"),
    ("intdiv", "int"),
    ("interface_exists", "bool"),
    ("intl_error_name", "string"),
    ("intl_get_error_code", "int"),
    ("intl_get_error_message", "string"),
    ("intl_is_failure", "bool"),
    ("intlcal_add", "bool"),
    ("intlcal_after", "bool"),
    ("intlcal_before", "bool"),
    ("intlcal_clear", "bool"),
    ("intlcal_equals", "bool"),
    ("intlcal_get_available_locales", "array"),
    ("intlcal_get_now", "float"),
    ("intlcal_get_repeated_wall_time_option", "int"),
    ("intlcal_get_skipped_wall_time_option", "int"),
    ("intlcal_get_type", "string"),
    ("intlcal_in_daylight_time", "bool"),
    ("intlcal_is_equivalent_to", "bool"),
    ("intlcal_is_lenient", "bool"),
    ("intlcal_is_set", "bool"),
    ("intlcal_is_weekend", "bool"),
    ("intlcal_roll", "bool"),
    ("intlcal_set", "bool"),
    ("intlcal_set_first_day_of_week", "bool"),
    ("intlcal_set_lenient", "bool"),
    ("intlcal_set_repeated_wall_time_option", "bool"),
    ("intlcal_set_skipped_wall_time_option", "bool"),
    ("intlcal_set_time", "bool"),
    ("intlcal_set_time_zone", "bool"),
    ("intltz_count_equivalent_ids", "int|false"),
    ("intltz_get_canonical_id", "string|false"),
    ("intltz_get_display_name", "string|false"),
    ("intltz_get_dst_savings", "int"),
    ("intltz_get_equivalent_id", "string|false"),
    ("intltz_get_error_code", "int|false"),
    ("intltz_get_error_message", "string|false"),
    ("intltz_get_id", "string|false"),
    ("intltz_get_raw_offset", "int"),
    ("intltz_get_tz_data_version", "string|false"),
    ("intltz_has_same_rules", "bool"),
    ("intltz_use_daylight_time", "bool"),
    ("intval", "int"),
    ("ip2long", "int|false"),
    ("iptcembed", "string|bool"),
    ("iptcparse", "false|array<string, list<string>>"),
    ("is_a", "bool"),
    ("is_array", "bool"),
    ("is_bool", "bool"),
    ("is_callable", "bool"),
    ("is_countable", "bool"),
    ("is_dir", "bool"),
    ("is_double", "bool"),
    ("is_executable", "bool"),
    ("is_file", "bool"),
    ("is_finite", "bool"),
    ("is_float", "bool"),
    ("is_infinite", "bool"),
    ("is_int", "bool"),
    ("is_integer", "bool"),
    ("is_iterable", "bool"),
    ("is_link", "bool"),
    ("is_long", "bool"),
    ("is_nan", "bool"),
    ("is_null", "bool"),
    ("is_numeric", "bool"),
    ("is_object", "bool"),
    ("is_readable", "bool"),
    ("is_resource", "bool"),
    ("is_scalar", "bool"),
    ("is_soap_fault", "bool"),
    ("is_string", "bool"),
    ("is_subclass_of", "bool"),
    ("is_uploaded_file", "bool"),
    ("is_writable", "bool"),
    ("is_writeable", "bool"),
    ("iterator_apply", "positive-int|0"),
    ("iterator_count", "positive-int|0"),
    ("iterator_to_array", "array"),
    ("jdmonthname", "string"),
    ("jdtofrench", "string"),
    ("jdtogregorian", "string"),
    ("jdtojewish", "string"),
    ("jdtojulian", "string"),
    ("jdtounix", "int|false"),
    ("jewishtojd", "int"),
    ("join", "string"),
    ("json_encode", "non-empty-string|false"),
    ("json_last_error_msg", "string"),
    ("juliantojd", "int"),
    ("key", "int|string|null"),
    ("key_exists", "bool"),
    ("krsort", "bool"),
    ("ksort", "bool"),
    ("lcfirst", "string"),
    ("lcg_value", "float"),
    ("lchgrp", "bool"),
    ("lchown", "bool"),
    ("ldap_add", "bool"),
    ("ldap_bind", "bool"),
    ("ldap_close", "bool"),
    ("ldap_count_entries", "int"),
    ("ldap_delete", "bool"),
    ("ldap_dn2ufn", "string|false"),
    ("ldap_err2str", "string"),
    ("ldap_errno", "int"),
    ("ldap_error", "string"),
    ("ldap_escape", "string"),
    ("ldap_exop_passwd", "string|bool"),
    ("ldap_exop_whoami", "string|bool"),
    ("ldap_explode_dn", "false|array"),
    ("ldap_first_attribute", "string|false"),
    ("ldap_free_result", "bool"),
    ("ldap_get_attributes", "array"),
    ("ldap_get_dn", "string|false"),
    ("ldap_get_entries", "false|array"),
    ("ldap_get_option", "bool"),
    ("ldap_get_values", "false|array"),
    ("ldap_get_values_len", "false|array"),
    ("ldap_mod_add", "bool"),
    ("ldap_mod_del", "bool"),
    ("ldap_mod_replace", "bool"),
    ("ldap_modify", "bool"),
    ("ldap_modify_batch", "bool"),
    ("ldap_next_attribute", "string|false"),
    ("ldap_parse_exop", "bool"),
    ("ldap_parse_reference", "bool"),
    ("ldap_parse_result", "bool"),
    ("ldap_rename", "bool"),
    ("ldap_sasl_bind", "bool"),
    ("ldap_set_option", "bool"),
    ("ldap_set_rebind_proc", "bool"),
    ("ldap_start_tls", "bool"),
    ("ldap_unbind", "bool"),
    ("levenshtein", "int"),
    ("libxml_disable_entity_loader", "bool"),
    ("libxml_get_errors", "list<libxmlerror>"),
    ("libxml_set_external_entity_loader", "bool"),
    ("libxml_use_internal_errors", "bool"),
    ("link", "bool"),
    ("linkinfo", "int|false"),
    ("locale_accept_from_http", "non-empty-string|false"),
    ("locale_canonicalize", "non-empty-string|null"),
    ("locale_compose", "string|false"),
    ("locale_filter_matches", "bool|null"),
    ("locale_get_all_variants", "null|array"),
    ("locale_get_default", "non-empty-string"),
    ("locale_get_display_language", "non-empty-string|false"),
    ("locale_get_display_name", "non-empty-string|false"),
    ("locale_get_display_region", "string|false"),
    ("locale_get_display_script", "string|false"),
    ("locale_get_display_variant", "string|false"),
    ("locale_get_primary_language", "non-empty-string|null"),
    ("locale_get_region", "string|null"),
    ("locale_get_script", "string|null"),
    ("locale_lookup", "string|null"),
    ("locale_parse", "null|array"),
    ("locale_set_default", "bool"),
    ("localeconv", "array{currency_symbol: string, decimal_point: string, frac_digits: int, grouping: list<int>, int_curr_symbol: string, int_frac_digits: int, mon_decimal_point: string, mon_grouping: array<int, int>, mon_thousands_sep: string, n_cs_precedes: int, n_sep_by_space: int, n_sign_posn: int, negative_sign: string, p_cs_precedes: int, p_sep_by_space: int, p_sign_posn: int, positive_sign: string, thousands_sep: string}"),
    ("localtime", "array"),
    ("log", "float"),
    ("log10", "float"),
    ("log1p", "float"),
    ("long2ip", "string|false"),
    ("lstat", "false|array"),
    ("ltrim", "string"),
    ("mail", "bool"),
    ("mb_check_encoding", "bool"),
    ("mb_chr", "string|false"),
    ("mb_convert_case", "string"),
    ("mb_convert_encoding", "string|false|array"),
    ("mb_convert_kana", "string"),
    ("mb_convert_variables", "string|false"),
    ("mb_decode_mimeheader", "string"),
    ("mb_decode_numericentity", "string|false"),
    ("mb_detect_encoding", "string|false"),
    ("mb_encode_mimeheader", "string"),
    ("mb_encode_numericentity", "string"),
    ("mb_encoding_aliases", "list<non-falsy-string>"),
    ("mb_ereg_match", "bool"),
    ("mb_ereg_replace", "string|false|null"),
    ("mb_ereg_replace_callback", "string|false|null"),
    ("mb_ereg_search", "bool"),
    ("mb_ereg_search_getpos", "int"),
    ("mb_ereg_search_getregs", "false|array"),
    ("mb_ereg_search_init", "bool"),
    ("mb_ereg_search_pos", "false|array"),
    ("mb_ereg_search_regs", "false|array"),
    ("mb_ereg_search_setpos", "bool"),
    ("mb_http_output", "string|bool"),
    ("mb_internal_encoding", "string|bool"),
    ("mb_language", "string|bool"),
    ("mb_lcfirst", "string"),
    ("mb_list_encodings", "non-empty-list<non-falsy-string>"),
    ("mb_ord", "int|false"),
    ("mb_output_handler", "string"),
    ("mb_parse_str", "bool"),
    ("mb_preferred_mime_name", "string|false"),
    ("mb_regex_encoding", "string|bool"),
    ("mb_regex_set_options", "string"),
    ("mb_scrub", "string"),
    ("mb_send_mail", "bool"),
    ("mb_split", "false|list<string>"),
    ("mb_str_split", "list<string>"),
    ("mb_strcut", "string"),
    ("mb_strimwidth", "string"),
    ("mb_stripos", "positive-int|0|false"),
    ("mb_stristr", "string|false"),
    ("mb_strlen", "positive-int|0"),
    ("mb_strpos", "positive-int|0|false"),
    ("mb_strrchr", "string|false"),
    ("mb_strrichr", "string|false"),
    ("mb_strripos", "positive-int|0|false"),
    ("mb_strrpos", "positive-int|0|false"),
    ("mb_strstr", "string|false"),
    ("mb_strtolower", "lowercase-string"),
    ("mb_strtoupper", "uppercase-string"),
    ("mb_strwidth", "positive-int|0"),
    ("mb_substr", "string"),
    ("mb_substr_count", "positive-int|0"),
    ("mb_ucfirst", "string"),
    ("memory_get_peak_usage", "positive-int"),
    ("memory_get_usage", "positive-int"),
    ("metaphone", "string"),
    ("method_exists", "bool"),
    ("mhash", "string|false"),
    ("mhash_count", "int"),
    ("mhash_get_block_size", "int|false"),
    ("mhash_get_hash_name", "string|false"),
    ("mhash_keygen_s2k", "string|false"),
    ("mime_content_type", "string|false"),
    ("mkdir", "bool"),
    ("mktime", "int|false"),
    ("move_uploaded_file", "bool"),
    ("msg_queue_exists", "bool"),
    ("msg_receive", "bool"),
    ("msg_remove_queue", "bool"),
    ("msg_send", "bool"),
    ("msg_set_queue", "bool"),
    ("msg_stat_queue", "false|array"),
    ("msgfmt_format", "string|false"),
    ("msgfmt_format_message", "string|false"),
    ("msgfmt_get_error_code", "int"),
    ("msgfmt_get_error_message", "string"),
    ("msgfmt_get_locale", "string"),
    ("msgfmt_get_pattern", "string|false"),
    ("msgfmt_parse", "false|array"),
    ("msgfmt_parse_message", "false|array"),
    ("msgfmt_set_pattern", "bool"),
    ("mt_getrandmax", "int"),
    ("mt_rand", "int"),
    ("mysqli_affected_rows", "int<-1, 9223372036854775807>|numeric-string"),
    ("mysqli_autocommit", "bool"),
    ("mysqli_begin_transaction", "bool"),
    ("mysqli_change_user", "bool"),
    ("mysqli_character_set_name", "string"),
    ("mysqli_close", "bool"),
    ("mysqli_commit", "bool"),
    ("mysqli_connect_errno", "int"),
    ("mysqli_connect_error", "string|null"),
    ("mysqli_data_seek", "bool"),
    ("mysqli_debug", "bool"),
    ("mysqli_dump_debug_info", "bool"),
    ("mysqli_errno", "int"),
    ("mysqli_error", "string|null"),
    ("mysqli_error_list", "array"),
    ("mysqli_fetch_all", "list<array>"),
    ("mysqli_fetch_array", "false|null|array"),
    ("mysqli_fetch_assoc", "false|null|array<string, int|float|string|null>"),
    ("mysqli_fetch_column", "int|float|string|false|null"),
    ("mysqli_fetch_fields", "list<stdclass&object>"),
    ("mysqli_fetch_lengths", "false|array"),
    ("mysqli_field_count", "int"),
    ("mysqli_field_seek", "bool"),
    ("mysqli_field_tell", "int"),
    ("mysqli_get_client_info", "string"),
    ("mysqli_get_client_stats", "false|array"),
    ("mysqli_get_client_version", "int"),
    ("mysqli_get_connection_stats", "false|array"),
    ("mysqli_get_host_info", "string"),
    ("mysqli_get_links_stats", "array"),
    ("mysqli_get_proto_info", "int"),
    ("mysqli_get_server_info", "string"),
    ("mysqli_get_server_version", "int"),
    ("mysqli_info", "string|null"),
    ("mysqli_insert_id", "int|string"),
    ("mysqli_kill", "bool"),
    ("mysqli_more_results", "bool"),
    ("mysqli_multi_query", "bool"),
    ("mysqli_next_result", "bool"),
    ("mysqli_num_fields", "int"),
    ("mysqli_num_rows", "non-negative-int|numeric-string"),
    ("mysqli_options", "bool"),
    ("mysqli_ping", "bool"),
    ("mysqli_poll", "int|false"),
    ("mysqli_real_connect", "bool"),
    ("mysqli_real_escape_string", "string"),
    ("mysqli_real_query", "bool"),
    ("mysqli_refresh", "bool"),
    ("mysqli_release_savepoint", "bool"),
    ("mysqli_report", "bool"),
    ("mysqli_rollback", "bool"),
    ("mysqli_savepoint", "bool"),
    ("mysqli_select_db", "bool"),
    ("mysqli_set_charset", "bool"),
    ("mysqli_sqlstate", "string"),
    ("mysqli_ssl_set", "bool"),
    ("mysqli_stat", "string|false"),
    ("mysqli_stmt_affected_rows", "int<-1, 9223372036854775807>|numeric-string"),
    ("mysqli_stmt_attr_get", "int|false"),
    ("mysqli_stmt_attr_set", "bool"),
    ("mysqli_stmt_bind_param", "bool"),
    ("mysqli_stmt_bind_result", "bool"),
    ("mysqli_stmt_close", "bool"),
    ("mysqli_stmt_errno", "int"),
    ("mysqli_stmt_error", "string"),
    ("mysqli_stmt_error_list", "list<array{errno: int, error: string, sqlstate: string}>"),
    ("mysqli_stmt_execute", "bool"),
    ("mysqli_stmt_fetch", "bool|null"),
    ("mysqli_stmt_field_count", "positive-int|0"),
    ("mysqli_stmt_more_results", "bool"),
    ("mysqli_stmt_next_result", "bool"),
    ("mysqli_stmt_param_count", "positive-int|0"),
    ("mysqli_stmt_prepare", "bool"),
    ("mysqli_stmt_reset", "bool"),
    ("mysqli_stmt_send_long_data", "bool"),
    ("mysqli_stmt_sqlstate", "non-empty-string"),
    ("mysqli_stmt_store_result", "bool"),
    ("mysqli_thread_id", "int"),
    ("mysqli_thread_safe", "bool"),
    ("mysqli_warning_count", "int"),
    ("natcasesort", "bool"),
    ("natsort", "bool"),
    ("net_get_interfaces", "false|array"),
    ("ngettext", "string"),
    ("nl2br", "string"),
    ("nl_langinfo", "string|false"),
    ("normalizer_get_raw_decomposition", "string|null"),
    ("normalizer_is_normalized", "bool"),
    ("normalizer_normalize", "string|false"),
    ("number_format", "non-empty-string"),
    ("numfmt_format", "string|false"),
    ("numfmt_format_currency", "string|false"),
    ("numfmt_get_attribute", "int|false"),
    ("numfmt_get_error_code", "int"),
    ("numfmt_get_error_message", "string"),
    ("numfmt_get_locale", "string|false"),
    ("numfmt_get_pattern", "string|false"),
    ("numfmt_get_symbol", "string|false"),
    ("numfmt_get_text_attribute", "string|false"),
    ("numfmt_parse", "float|false"),
    ("numfmt_parse_currency", "float|false"),
    ("numfmt_set_attribute", "bool"),
    ("numfmt_set_pattern", "bool"),
    ("numfmt_set_symbol", "bool"),
    ("numfmt_set_text_attribute", "bool"),
    ("ob_clean", "bool"),
    ("ob_end_clean", "bool"),
    ("ob_end_flush", "bool"),
    ("ob_flush", "bool"),
    ("ob_get_clean", "string|false"),
    ("ob_get_contents", "string|false"),
    ("ob_get_flush", "string|false"),
    ("ob_get_length", "int|false"),
    ("ob_get_level", "non-negative-int"),
    ("ob_get_status", "array"),
    ("ob_gzhandler", "string|false"),
    ("ob_start", "bool"),
    ("octdec", "int|float"),
    ("odbc_binmode", "bool"),
    ("odbc_commit", "bool"),
    ("odbc_cursor", "string|false"),
    ("odbc_error", "string"),
    ("odbc_errormsg", "string"),
    ("odbc_execute", "bool"),
    ("odbc_fetch_array", "false|array"),
    ("odbc_fetch_into", "int|false"),
    ("odbc_fetch_row", "bool"),
    ("odbc_field_len", "int|false"),
    ("odbc_field_name", "string|false"),
    ("odbc_field_num", "int|false"),
    ("odbc_field_precision", "int|false"),
    ("odbc_field_scale", "int|false"),
    ("odbc_field_type", "string|false"),
    ("odbc_free_result", "bool"),
    ("odbc_longreadlen", "bool"),
    ("odbc_next_result", "bool"),
    ("odbc_num_fields", "int"),
    ("odbc_num_rows", "int"),
    ("odbc_result_all", "int|false"),
    ("odbc_rollback", "bool"),
    ("odbc_setoption", "bool"),
    ("opcache_compile_file", "bool"),
    ("opcache_get_configuration", "false|array{blacklist: list<string>, directives: array{'opcache.blacklist_filename': string, 'opcache.cache_id'?: string, 'opcache.dups_fix': bool, 'opcache.enable': bool, 'opcache.enable_cli': bool, 'opcache.enable_file_override': bool, 'opcache.error_log': string, 'opcache.file_cache': string, 'opcache.file_cache_consistency_checks': bool, 'opcache.file_cache_fallback'?: bool, 'opcache.file_cache_only': bool, 'opcache.file_update_protection': int, 'opcache.force_restart_timeout': int, 'opcache.huge_code_pages'?: bool, 'opcache.interned_strings_buffer': int, 'opcache.jit'?: string, 'opcache.jit_bisect_limit'?: int, 'opcache.jit_blacklist_root_trace'?: int, 'opcache.jit_blacklist_side_trace'?: int, 'opcache.jit_buffer_size'?: int, 'opcache.jit_debug'?: int, 'opcache.jit_hot_func'?: int, 'opcache.jit_hot_loop'?: int, 'opcache.jit_hot_return'?: int, 'opcache.jit_hot_side_exit'?: int, 'opcache.jit_max_exit_counters'?: int, 'opcache.jit_max_loop_unrolls'?: int, 'opcache.jit_max_polymorphic_calls'?: int, 'opcache.jit_max_recursive_calls'?: int, 'opcache.jit_max_recursive_returns'?: int, 'opcache.jit_max_root_traces'?: int, 'opcache.jit_max_side_traces'?: int, 'opcache.jit_max_trace_length'?: int, 'opcache.jit_prof_threshold'?: float, 'opcache.lockfile_path'?: string, 'opcache.log_verbosity_level': int, 'opcache.max_accelerated_files': int, 'opcache.max_file_size': int, 'opcache.max_wasted_percentage': float, 'opcache.memory_consumption': int, 'opcache.mmap_base'?: string, 'opcache.opt_debug_level': int, 'opcache.optimization_level': int, 'opcache.preferred_memory_model': string, 'opcache.preload': string, 'opcache.preload_user'?: string, 'opcache.protect_memory': bool, 'opcache.record_warnings': bool, 'opcache.restrict_api': string, 'opcache.revalidate_freq': int, 'opcache.revalidate_path': bool, 'opcache.save_comments': bool, 'opcache.use_cwd': bool, 'opcache.validate_permission': bool, 'opcache.validate_root'?: bool, 'opcache.validate_timestamps': bool}, version: array{opcache_product_name: non-empty-string, version: non-empty-string}}"),
    ("opcache_get_status", "false|array"),
    ("opcache_invalidate", "bool"),
    ("opcache_is_script_cached", "bool"),
    ("opcache_reset", "bool"),
    ("openlog", "bool"),
    ("openssl_cipher_iv_length", "int|false"),
    ("openssl_csr_export", "bool"),
    ("openssl_csr_export_to_file", "bool"),
    ("openssl_csr_get_subject", "false|array"),
    ("openssl_decrypt", "string|false"),
    ("openssl_dh_compute_key", "string|false"),
    ("openssl_digest", "string|false"),
    ("openssl_encrypt", "string|false"),
    ("openssl_error_string", "string|false"),
    ("openssl_get_cert_locations", "array<non-falsy-string, string>"),
    ("openssl_get_cipher_methods", "list<string>"),
    ("openssl_get_curve_names", "false|list<string>"),
    ("openssl_get_md_methods", "list<string>"),
    ("openssl_open", "bool"),
    ("openssl_pbkdf2", "string|false"),
    ("openssl_pkcs12_export", "bool"),
    ("openssl_pkcs12_export_to_file", "bool"),
    ("openssl_pkcs12_read", "bool"),
    ("openssl_pkcs7_decrypt", "bool"),
    ("openssl_pkcs7_encrypt", "bool"),
    ("openssl_pkcs7_read", "bool"),
    ("openssl_pkcs7_sign", "bool"),
    ("openssl_pkcs7_verify", "int|bool"),
    ("openssl_pkey_derive", "string|false"),
    ("openssl_pkey_export", "bool"),
    ("openssl_pkey_export_to_file", "bool"),
    ("openssl_pkey_get_details", "false|array"),
    ("openssl_private_decrypt", "bool"),
    ("openssl_private_encrypt", "bool"),
    ("openssl_public_decrypt", "bool"),
    ("openssl_public_encrypt", "bool"),
    ("openssl_random_pseudo_bytes", "string|false"),
    ("openssl_seal", "int|false"),
    ("openssl_sign", "bool"),
    ("openssl_spki_export", "string|false|null"),
    ("openssl_spki_export_challenge", "string|false|null"),
    ("openssl_spki_new", "string|false|null"),
    ("openssl_spki_verify", "bool"),
    ("openssl_verify", "-1|0|1|false"),
    ("openssl_x509_check_private_key", "bool"),
    ("openssl_x509_checkpurpose", "int|bool"),
    ("openssl_x509_export", "bool"),
    ("openssl_x509_export_to_file", "bool"),
    ("openssl_x509_fingerprint", "string|false"),
    ("openssl_x509_parse", "false|array"),
    ("ord", "int<0, 255>"),
    ("output_add_rewrite_var", "bool"),
    ("output_reset_rewrite_vars", "bool"),
    ("pack", "string"),
    ("parse_ini_file", "false|array"),
    ("parse_ini_string", "false|array"),
    ("parse_url", "int|string|false|null|array"),
    ("password_algos", "list<string>"),
    ("password_get_info", "array"),
    ("password_hash", "non-empty-string"),
    ("password_needs_rehash", "bool"),
    ("password_verify", "bool"),
    ("pathinfo", "string|array"),
    ("pclose", "int"),
    ("pcntl_alarm", "int"),
    ("pcntl_async_signals", "bool"),
    ("pcntl_errno", "int"),
    ("pcntl_exec", "bool"),
    ("pcntl_fork", "int"),
    ("pcntl_get_last_error", "int"),
    ("pcntl_getpriority", "int|false"),
    ("pcntl_setpriority", "bool"),
    ("pcntl_signal", "bool"),
    ("pcntl_signal_dispatch", "bool"),
    ("pcntl_sigprocmask", "bool"),
    ("pcntl_strerror", "string"),
    ("pcntl_wait", "int"),
    ("pcntl_waitpid", "int"),
    ("pcntl_wexitstatus", "int|false"),
    ("pcntl_wifcontinued", "bool"),
    ("pcntl_wifexited", "bool"),
    ("pcntl_wifsignaled", "bool"),
    ("pcntl_wifstopped", "bool"),
    ("pcntl_wstopsig", "int|false"),
    ("pcntl_wtermsig", "int|false"),
    ("pdo_drivers", "array"),
    ("pg_affected_rows", "int"),
    ("pg_cancel_query", "bool"),
    ("pg_client_encoding", "string"),
    ("pg_close", "bool"),
    ("pg_connect_poll", "int"),
    ("pg_connection_busy", "bool"),
    ("pg_connection_reset", "bool"),
    ("pg_connection_status", "int"),
    ("pg_consume_input", "bool"),
    ("pg_convert", "false|array"),
    ("pg_copy_from", "bool"),
    ("pg_copy_to", "false|array"),
    ("pg_dbname", "string"),
    ("pg_end_copy", "bool"),
    ("pg_fetch_all", "array<non-empty-list<string|null>>"),
    ("pg_fetch_all_columns", "false|array"),
    ("pg_fetch_array", "false|array"),
    ("pg_fetch_assoc", "false|non-empty-array<string, mixed>"),
    ("pg_fetch_row", "false|non-empty-list<mixed>"),
    ("pg_field_name", "string|false"),
    ("pg_field_num", "int"),
    ("pg_field_size", "int"),
    ("pg_field_type", "string"),
    ("pg_free_result", "bool"),
    ("pg_get_notify", "false|array"),
    ("pg_get_pid", "int|false"),
    ("pg_host", "string"),
    ("pg_last_error", "string"),
    ("pg_lo_close", "bool"),
    ("pg_lo_read", "string|false"),
    ("pg_lo_read_all", "int"),
    ("pg_lo_seek", "bool"),
    ("pg_lo_tell", "int"),
    ("pg_lo_truncate", "bool"),
    ("pg_lo_unlink", "bool"),
    ("pg_lo_write", "int|false"),
    ("pg_meta_data", "false|array"),
    ("pg_num_fields", "int"),
    ("pg_num_rows", "int"),
    ("pg_options", "string"),
    ("pg_ping", "bool"),
    ("pg_result_error", "string|false"),
    ("pg_result_error_field", "string|false|null"),
    ("pg_result_seek", "bool"),
    ("pg_trace", "bool"),
    ("pg_transaction_status", "int"),
    ("pg_unescape_bytea", "string"),
    ("pg_version", "array"),
    ("php_ini_loaded_file", "non-empty-string|false"),
    ("php_ini_scanned_files", "string|false"),
    ("php_sapi_name", "non-empty-string|false"),
    ("php_strip_whitespace", "string"),
    ("php_uname", "string"),
    ("phpcredits", "bool"),
    ("phpinfo", "bool"),
    ("pi", "float"),
    ("posix_access", "bool"),
    ("posix_ctermid", "string|false"),
    ("posix_errno", "int"),
    ("posix_get_last_error", "int"),
    ("posix_getcwd", "string|false"),
    ("posix_getegid", "int"),
    ("posix_geteuid", "int"),
    ("posix_getgid", "int"),
    ("posix_getgrgid", "false|array{gid: int, members: list<string>, name: string, passwd: string}"),
    ("posix_getgrnam", "false|array{gid: int, members: list<string>, name: string, passwd: string}"),
    ("posix_getgroups", "false|list<int>"),
    ("posix_getlogin", "string|false"),
    ("posix_getpgid", "int|false"),
    ("posix_getpgrp", "int"),
    ("posix_getpid", "int"),
    ("posix_getppid", "int"),
    ("posix_getpwnam", "false|array{dir: string, gecos: string, gid: int, name: string, passwd: string, shell: string, uid: int}"),
    ("posix_getpwuid", "false|array{dir: string, gecos: string, gid: int, name: string, passwd: string, shell: string, uid: int}"),
    ("posix_getrlimit", "false|array"),
    ("posix_getsid", "int|false"),
    ("posix_getuid", "int"),
    ("posix_initgroups", "bool"),
    ("posix_isatty", "bool"),
    ("posix_kill", "bool"),
    ("posix_mkfifo", "bool"),
    ("posix_mknod", "bool"),
    ("posix_setegid", "bool"),
    ("posix_seteuid", "bool"),
    ("posix_setgid", "bool"),
    ("posix_setpgid", "bool"),
    ("posix_setrlimit", "bool"),
    ("posix_setsid", "int"),
    ("posix_setuid", "bool"),
    ("posix_strerror", "string"),
    ("posix_times", "false|array"),
    ("posix_ttyname", "string|false"),
    ("posix_uname", "false|array"),
    ("preg_filter", "string|null|array"),
    ("preg_grep", "false|array"),
    ("preg_last_error", "int"),
    ("preg_match", "0|1|false"),
    ("preg_match_all", "positive-int|0|false"),
    ("preg_quote", "string"),
    ("preg_replace", "string|null|array"),
    ("preg_replace_callback", "string|null|array"),
    ("preg_replace_callback_array", "string|null|array"),
    ("preg_split", "false|list<string>|list<list{string, non-negative-int}>"),
    ("print_r", "string|true"),
    ("printf", "int"),
    ("proc_close", "int"),
    ("proc_get_status", "array{command: string, exitcode: int, pid: int, running: bool, signaled: bool, stopped: bool, stopsig: int, termsig: int}"),
    ("proc_nice", "bool"),
    ("proc_terminate", "bool"),
    ("property_exists", "bool"),
    ("putenv", "bool"),
    ("quoted_printable_decode", "string"),
    ("quoted_printable_encode", "string"),
    ("quotemeta", "string"),
    ("rad2deg", "float"),
    ("rand", "int"),
    ("random_bytes", "non-empty-string"),
    ("random_int", "int"),
    ("range", "array"),
    ("rawurldecode", "string"),
    ("rawurlencode", "string"),
    ("readdir", "non-empty-string|false"),
    ("readfile", "positive-int|0|false"),
    ("readgzfile", "positive-int|0|false"),
    ("readline", "string|false"),
    ("readline_add_history", "bool"),
    ("readline_callback_handler_install", "bool"),
    ("readline_callback_handler_remove", "bool"),
    ("readline_clear_history", "bool"),
    ("readline_completion_function", "bool"),
    ("readline_read_history", "bool"),
    ("readline_write_history", "bool"),
    ("readlink", "string|false"),
    ("realpath", "non-empty-string|false"),
    ("realpath_cache_get", "array"),
    ("realpath_cache_size", "int"),
    ("register_tick_function", "bool"),
    ("rename", "bool"),
    ("resourcebundle_count", "int"),
    ("resourcebundle_get_error_code", "int"),
    ("resourcebundle_get_error_message", "string"),
    ("resourcebundle_locales", "false|array"),
    ("restore_error_handler", "true"),
    ("restore_exception_handler", "true"),
    ("rewind", "bool"),
    ("rmdir", "bool"),
    ("round", "float"),
    ("rsort", "bool"),
    ("rtrim", "string"),
    ("scandir", "false|list<string>"),
    ("sem_acquire", "bool"),
    ("sem_release", "bool"),
    ("sem_remove", "bool"),
    ("serialize", "string"),
    ("session_abort", "bool"),
    ("session_cache_expire", "int|false"),
    ("session_cache_limiter", "string|false"),
    ("session_commit", "bool"),
    ("session_create_id", "string|false"),
    ("session_decode", "bool"),
    ("session_destroy", "bool"),
    ("session_encode", "string|false"),
    ("session_gc", "int|false"),
    ("session_get_cookie_params", "array{domain: string, httponly: bool, lifetime: positive-int|0, partitioned: bool, path: non-falsy-string, samesite: 'Lax'|'None'|'Strict'|'lax'|'none'|'strict', secure: bool}"),
    ("session_id", "string|false"),
    ("session_module_name", "string|false"),
    ("session_name", "non-falsy-string|false"),
    ("session_regenerate_id", "bool"),
    ("session_reset", "bool"),
    ("session_save_path", "string|false"),
    ("session_set_cookie_params", "bool"),
    ("session_set_save_handler", "bool"),
    ("session_start", "bool"),
    ("session_unset", "bool"),
    ("session_write_close", "bool"),
    ("set_file_buffer", "int"),
    ("set_include_path", "string|false"),
    ("set_time_limit", "bool"),
    ("setcookie", "bool"),
    ("setlocale", "string|false"),
    ("setrawcookie", "bool"),
    ("settype", "bool"),
    ("shell_exec", "string|false|null"),
    ("shm_detach", "bool"),
    ("shm_has_var", "bool"),
    ("shm_put_var", "bool"),
    ("shm_remove", "bool"),
    ("shm_remove_var", "bool"),
    ("shmop_delete", "bool"),
    ("shmop_read", "string"),
    ("shmop_size", "int"),
    ("shmop_write", "int"),
    ("shuffle", "bool"),
    ("similar_text", "int"),
    ("sin", "float"),
    ("sinh", "float"),
    ("sizeof", "int"),
    ("sleep", "int|false"),
    ("snmp2_getnext", "string|false"),
    ("snmp2_real_walk", "false|array<string, string>"),
    ("snmp2_set", "bool"),
    ("snmp2_walk", "false|array"),
    ("snmp3_getnext", "string|false"),
    ("snmp3_real_walk", "false|array"),
    ("snmp3_set", "bool"),
    ("snmp3_walk", "false|array"),
    ("snmp_get_quick_print", "bool"),
    ("snmp_get_valueretrieval", "int"),
    ("snmp_read_mib", "bool"),
    ("snmp_set_enum_print", "true"),
    ("snmp_set_oid_numeric_print", "true"),
    ("snmp_set_oid_output_format", "true"),
    ("snmp_set_quick_print", "true"),
    ("snmp_set_valueretrieval", "true"),
    ("snmpgetnext", "string|false"),
    ("snmprealwalk", "false|array<string, string>"),
    ("snmpset", "bool"),
    ("snmpwalk", "false|array<string, string>"),
    ("snmpwalkoid", "false|array<string, string>"),
    ("socket_addrinfo_explain", "array"),
    ("socket_bind", "bool"),
    ("socket_connect", "bool"),
    ("socket_create_pair", "bool"),
    ("socket_getpeername", "bool"),
    ("socket_getsockname", "bool"),
    ("socket_last_error", "int"),
    ("socket_listen", "bool"),
    ("socket_read", "string|false"),
    ("socket_recv", "int|false"),
    ("socket_recvfrom", "int|false"),
    ("socket_recvmsg", "int|false"),
    ("socket_select", "int|false"),
    ("socket_send", "int|false"),
    ("socket_sendmsg", "int|false"),
    ("socket_sendto", "int|false"),
    ("socket_set_block", "bool"),
    ("socket_set_nonblock", "bool"),
    ("socket_set_option", "bool"),
    ("socket_shutdown", "bool"),
    ("socket_strerror", "string"),
    ("socket_write", "int|false"),
    ("sodium_base642bin", "string"),
    ("sodium_bin2base64", "string"),
    ("sodium_bin2hex", "string"),
    ("sodium_compare", "int"),
    ("sodium_crypto_aead_aes256gcm_decrypt", "string|false"),
    ("sodium_crypto_aead_aes256gcm_encrypt", "string"),
    ("sodium_crypto_aead_aes256gcm_is_available", "bool"),
    ("sodium_crypto_aead_aes256gcm_keygen", "string"),
    ("sodium_crypto_aead_chacha20poly1305_decrypt", "string|false"),
    ("sodium_crypto_aead_chacha20poly1305_encrypt", "string"),
    ("sodium_crypto_aead_chacha20poly1305_ietf_decrypt", "string|false"),
    ("sodium_crypto_aead_chacha20poly1305_ietf_encrypt", "string"),
    ("sodium_crypto_aead_chacha20poly1305_ietf_keygen", "string"),
    ("sodium_crypto_aead_chacha20poly1305_keygen", "string"),
    ("sodium_crypto_aead_xchacha20poly1305_ietf_decrypt", "string|false"),
    ("sodium_crypto_aead_xchacha20poly1305_ietf_encrypt", "string"),
    ("sodium_crypto_aead_xchacha20poly1305_ietf_keygen", "string"),
    ("sodium_crypto_auth", "string"),
    ("sodium_crypto_auth_keygen", "string"),
    ("sodium_crypto_auth_verify", "bool"),
    ("sodium_crypto_box", "string"),
    ("sodium_crypto_box_keypair", "string"),
    ("sodium_crypto_box_keypair_from_secretkey_and_publickey", "string"),
    ("sodium_crypto_box_open", "string|false"),
    ("sodium_crypto_box_publickey", "string"),
    ("sodium_crypto_box_publickey_from_secretkey", "string"),
    ("sodium_crypto_box_seal", "string"),
    ("sodium_crypto_box_seal_open", "string|false"),
    ("sodium_crypto_box_secretkey", "string"),
    ("sodium_crypto_box_seed_keypair", "string"),
    ("sodium_crypto_generichash", "non-empty-string"),
    ("sodium_crypto_generichash_final", "non-empty-string"),
    ("sodium_crypto_generichash_init", "non-empty-string"),
    ("sodium_crypto_generichash_keygen", "non-empty-string"),
    ("sodium_crypto_generichash_update", "bool"),
    ("sodium_crypto_kdf_derive_from_key", "string"),
    ("sodium_crypto_kdf_keygen", "string"),
    ("sodium_crypto_kx_client_session_keys", "array<int, string>"),
    ("sodium_crypto_kx_keypair", "string"),
    ("sodium_crypto_kx_publickey", "string"),
    ("sodium_crypto_kx_secretkey", "string"),
    ("sodium_crypto_kx_seed_keypair", "string"),
    ("sodium_crypto_kx_server_session_keys", "array<int, string>"),
    ("sodium_crypto_pwhash", "string"),
    ("sodium_crypto_pwhash_scryptsalsa208sha256", "string"),
    ("sodium_crypto_pwhash_scryptsalsa208sha256_str", "string"),
    ("sodium_crypto_pwhash_scryptsalsa208sha256_str_verify", "bool"),
    ("sodium_crypto_pwhash_str", "string"),
    ("sodium_crypto_pwhash_str_needs_rehash", "bool"),
    ("sodium_crypto_pwhash_str_verify", "bool"),
    ("sodium_crypto_scalarmult", "string"),
    ("sodium_crypto_scalarmult_base", "string"),
    ("sodium_crypto_secretbox", "string"),
    ("sodium_crypto_secretbox_keygen", "string"),
    ("sodium_crypto_secretbox_open", "string|false"),
    ("sodium_crypto_secretstream_xchacha20poly1305_init_pull", "string"),
    ("sodium_crypto_secretstream_xchacha20poly1305_init_push", "array"),
    ("sodium_crypto_secretstream_xchacha20poly1305_keygen", "string"),
    ("sodium_crypto_secretstream_xchacha20poly1305_pull", "false|array"),
    ("sodium_crypto_secretstream_xchacha20poly1305_push", "string"),
    ("sodium_crypto_shorthash", "string"),
    ("sodium_crypto_shorthash_keygen", "string"),
    ("sodium_crypto_sign", "non-empty-string"),
    ("sodium_crypto_sign_detached", "non-empty-string"),
    ("sodium_crypto_sign_ed25519_pk_to_curve25519", "non-empty-string"),
    ("sodium_crypto_sign_ed25519_sk_to_curve25519", "non-empty-string"),
    ("sodium_crypto_sign_keypair", "non-empty-string"),
    ("sodium_crypto_sign_keypair_from_secretkey_and_publickey", "non-empty-string"),
    ("sodium_crypto_sign_open", "string|false"),
    ("sodium_crypto_sign_publickey", "non-empty-string"),
    ("sodium_crypto_sign_publickey_from_secretkey", "non-empty-string"),
    ("sodium_crypto_sign_secretkey", "non-empty-string"),
    ("sodium_crypto_sign_seed_keypair", "non-empty-string"),
    ("sodium_crypto_sign_verify_detached", "bool"),
    ("sodium_crypto_stream", "string"),
    ("sodium_crypto_stream_keygen", "string"),
    ("sodium_crypto_stream_xor", "string"),
    ("sodium_hex2bin", "string"),
    ("sodium_memcmp", "int"),
    ("sodium_pad", "string"),
    ("sodium_unpad", "string"),
    ("sort", "bool"),
    ("soundex", "string"),
    ("spl_autoload_extensions", "string"),
    ("spl_autoload_functions", "list<callable(string):void>"),
    ("spl_autoload_register", "bool"),
    ("spl_autoload_unregister", "bool"),
    ("spl_classes", "array"),
    ("spl_object_hash", "string"),
    ("spl_object_id", "int"),
    ("sprintf", "string"),
    ("sqrt", "float"),
    ("stat", "false|array"),
    ("str_contains", "bool"),
    ("str_decrement", "non-empty-string"),
    ("str_ends_with", "bool"),
    ("str_getcsv", "non-empty-list<string|null>"),
    ("str_increment", "non-falsy-string"),
    ("str_ireplace", "string|array<string>"),
    ("str_pad", "string"),
    ("str_repeat", "string"),
    ("str_replace", "string|array"),
    ("str_rot13", "string"),
    ("str_shuffle", "string"),
    ("str_split", "list<string>"),
    ("str_starts_with", "bool"),
    ("strcasecmp", "int<-1, 1>"),
    ("strchr", "string|false"),
    ("strcmp", "int<-1, 1>"),
    ("strcoll", "int<-1, 1>"),
    ("strcspn", "non-negative-int"),
    ("stream_context_get_options", "array"),
    ("stream_context_get_params", "array{notification: string, options: array<mixed>}"),
    ("stream_context_set_option", "bool"),
    ("stream_context_set_params", "bool"),
    ("stream_copy_to_stream", "int|false"),
    ("stream_filter_register", "bool"),
    ("stream_filter_remove", "bool"),
    ("stream_get_contents", "string|false"),
    ("stream_get_filters", "list<string>"),
    ("stream_get_line", "string|false"),
    ("stream_get_meta_data", "array{base64?: bool, blocked: bool, crypto?: array{alpn_protocol?: string, cipher_bits: int, cipher_name: string, cipher_version: string, protocol: string}, eof: bool, mediatype?: string, mode: string, seekable: bool, stream_type: string, timed_out: bool, unread_bytes: int, uri?: string, wrapper_data: mixed, wrapper_type: string}"),
    ("stream_get_transports", "list<string>"),
    ("stream_get_wrappers", "list<string>"),
    ("stream_is_local", "bool"),
    ("stream_isatty", "bool"),
    ("stream_resolve_include_path", "string|false"),
    ("stream_select", "int|false"),
    ("stream_set_blocking", "bool"),
    ("stream_set_chunk_size", "int|false"),
    ("stream_set_read_buffer", "int"),
    ("stream_set_timeout", "bool"),
    ("stream_set_write_buffer", "int"),
    ("stream_socket_enable_crypto", "0|bool"),
    ("stream_socket_get_name", "string|false"),
    ("stream_socket_pair", "resource[]|false"),
    ("stream_socket_recvfrom", "string|false"),
    ("stream_socket_sendto", "int|false"),
    ("stream_socket_shutdown", "bool"),
    ("stream_supports_lock", "bool"),
    ("stream_wrapper_register", "bool"),
    ("stream_wrapper_restore", "bool"),
    ("stream_wrapper_unregister", "bool"),
    ("strftime", "string|false"),
    ("strip_tags", "string"),
    ("stripcslashes", "string"),
    ("stripos", "positive-int|0|false"),
    ("stripslashes", "string"),
    ("stristr", "string|false"),
    ("strlen", "positive-int|0"),
    ("strnatcasecmp", "int<-1, 1>"),
    ("strnatcmp", "int<-1, 1>"),
    ("strncasecmp", "int<-1, 1>"),
    ("strncmp", "int<-1, 1>"),
    ("strpbrk", "string|false"),
    ("strpos", "positive-int|0|false"),
    ("strptime", "false|array"),
    ("strrchr", "string|false"),
    ("strrev", "string"),
    ("strripos", "int|false"),
    ("strrpos", "int|false"),
    ("strspn", "non-negative-int"),
    ("strstr", "string|false"),
    ("strtok", "non-empty-string|false"),
    ("strtolower", "lowercase-string"),
    ("strtotime", "int|false"),
    ("strtoupper", "uppercase-string"),
    ("strtr", "string"),
    ("strval", "string"),
    ("substr", "string"),
    ("substr_count", "positive-int|0"),
    ("substr_replace", "string|array"),
    ("symlink", "bool"),
    ("sys_get_temp_dir", "string"),
    ("sys_getloadavg", "false|list{float, float, float}"),
    ("syslog", "bool"),
    ("system", "string|false"),
    ("tan", "float"),
    ("tanh", "float"),
    ("tempnam", "non-falsy-string|false"),
    ("textdomain", "string"),
    ("tidy_access_count", "int"),
    ("tidy_clean_repair", "bool"),
    ("tidy_config_count", "int"),
    ("tidy_diagnose", "bool"),
    ("tidy_error_count", "int"),
    ("tidy_get_config", "array"),
    ("tidy_get_error_buffer", "string|false"),
    ("tidy_get_html_ver", "int"),
    ("tidy_get_opt_doc", "string|false"),
    ("tidy_get_output", "string"),
    ("tidy_get_release", "string"),
    ("tidy_get_status", "int"),
    ("tidy_is_xhtml", "bool"),
    ("tidy_is_xml", "bool"),
    ("tidy_repair_file", "string|false"),
    ("tidy_repair_string", "string|false"),
    ("tidy_warning_count", "int"),
    ("time", "positive-int"),
    ("time_nanosleep", "bool|array{nanoseconds: positive-int|0, seconds: positive-int|0}"),
    ("time_sleep_until", "bool"),
    ("timezone_abbreviations_list", "array<string, list<array{dst: bool, offset: int, timezone_id: string|null}>>"),
    ("timezone_identifiers_list", "list<string>"),
    ("timezone_location_get", "false|array{comments: string, country_code: string, latitude: float, longitude: float}"),
    ("timezone_name_from_abbr", "string|false"),
    ("timezone_name_get", "string"),
    ("timezone_offset_get", "int"),
    ("timezone_transitions_get", "false|list<array{abbr: string, isdst: bool, offset: int, time: string, ts: int}>"),
    ("timezone_version_get", "string"),
    ("token_get_all", "list<string|list{int, string, int}>"),
    ("token_name", "non-falsy-string"),
    ("touch", "bool"),
    ("trait_exists", "bool"),
    ("transliterator_get_error_code", "int|false"),
    ("transliterator_get_error_message", "string|false"),
    ("transliterator_list_ids", "false|list<string>"),
    ("transliterator_transliterate", "string|false"),
    ("trigger_error", "bool"),
    ("trim", "string"),
    ("uasort", "bool"),
    ("ucfirst", "string"),
    ("ucwords", "string"),
    ("uksort", "bool"),
    ("umask", "int"),
    ("uniqid", "non-empty-string"),
    ("unixtojd", "int|false"),
    ("unlink", "bool"),
    ("unpack", "false|array"),
    ("urldecode", "string"),
    ("urlencode", "string"),
    ("use_soap_error_handler", "bool"),
    ("usort", "bool"),
    ("utf8_decode", "string"),
    ("utf8_encode", "string"),
    ("var_export", "string|null"),
    ("vfprintf", "int"),
    ("vprintf", "int"),
    ("vsprintf", "string"),
    ("wordwrap", "string"),
    ("xml_get_current_byte_index", "int"),
    ("xml_get_current_column_number", "int"),
    ("xml_get_current_line_number", "int"),
    ("xml_get_error_code", "int"),
    ("xml_parse", "int"),
    ("xml_parser_free", "bool"),
    ("xml_parser_set_option", "bool"),
    ("xml_set_character_data_handler", "bool"),
    ("xml_set_default_handler", "bool"),
    ("xml_set_element_handler", "bool"),
    ("xml_set_end_namespace_decl_handler", "bool"),
    ("xml_set_external_entity_ref_handler", "bool"),
    ("xml_set_notation_decl_handler", "bool"),
    ("xml_set_object", "bool"),
    ("xml_set_processing_instruction_handler", "bool"),
    ("xml_set_start_namespace_decl_handler", "bool"),
    ("xml_set_unparsed_entity_decl_handler", "bool"),
    ("xmlwriter_end_attribute", "bool"),
    ("xmlwriter_end_cdata", "bool"),
    ("xmlwriter_end_comment", "bool"),
    ("xmlwriter_end_document", "bool"),
    ("xmlwriter_end_dtd", "bool"),
    ("xmlwriter_end_dtd_attlist", "bool"),
    ("xmlwriter_end_dtd_element", "bool"),
    ("xmlwriter_end_dtd_entity", "bool"),
    ("xmlwriter_end_element", "bool"),
    ("xmlwriter_end_pi", "bool"),
    ("xmlwriter_full_end_element", "bool"),
    ("xmlwriter_output_memory", "string"),
    ("xmlwriter_set_indent", "bool"),
    ("xmlwriter_set_indent_string", "bool"),
    ("xmlwriter_start_attribute", "bool"),
    ("xmlwriter_start_attribute_ns", "bool"),
    ("xmlwriter_start_cdata", "bool"),
    ("xmlwriter_start_comment", "bool"),
    ("xmlwriter_start_document", "bool"),
    ("xmlwriter_start_dtd", "bool"),
    ("xmlwriter_start_dtd_attlist", "bool"),
    ("xmlwriter_start_dtd_element", "bool"),
    ("xmlwriter_start_dtd_entity", "bool"),
    ("xmlwriter_start_element", "bool"),
    ("xmlwriter_start_element_ns", "bool"),
    ("xmlwriter_start_pi", "bool"),
    ("xmlwriter_text", "bool"),
    ("xmlwriter_write_attribute", "bool"),
    ("xmlwriter_write_attribute_ns", "bool"),
    ("xmlwriter_write_cdata", "bool"),
    ("xmlwriter_write_comment", "bool"),
    ("xmlwriter_write_dtd", "bool"),
    ("xmlwriter_write_dtd_attlist", "bool"),
    ("xmlwriter_write_dtd_element", "bool"),
    ("xmlwriter_write_dtd_entity", "bool"),
    ("xmlwriter_write_element", "bool"),
    ("xmlwriter_write_element_ns", "bool"),
    ("xmlwriter_write_pi", "bool"),
    ("xmlwriter_write_raw", "bool"),
    ("zend_version", "string"),
    ("zip_entry_close", "bool"),
    ("zip_entry_compressedsize", "int|false"),
    ("zip_entry_compressionmethod", "string|false"),
    ("zip_entry_filesize", "int|false"),
    ("zip_entry_name", "string|false"),
    ("zip_entry_open", "bool"),
    ("zip_entry_read", "string|false"),
    ("zlib_decode", "string|false"),
    ("zlib_encode", "string|false"),
    ("zlib_get_coding_type", "string|false"),
];

// The A11-shaped change oracle: names whose declared RETURN type moves between
// two adjacent supported minors, keyed to the minor it moved AT. A project whose
// declared PhpTarget is not wholly at or above that minor declines the row; an
// unknown target admits (the row is Asserted anyway, ADR-0069 §3).
//
// Listed independently of the table above: a name can be version-sensitive
// without carrying an admitted row, and the gate must stay complete either way.
// Sorted by key for binary search.

pub(crate) static RETURN_VERSION_SENSITIVE: &[(&str, (u16, u16))] = &[
    ("gc_status", (8, 3)),
    ("opcache_get_configuration", (8, 3)),
    ("session_get_cookie_params", (8, 5)),
    ("str_split", (8, 2)),
];

...
- **`steins transform throws-envelope` seeds `@throws` tags from proven escapes, so a repo can adopt the `throws-direct` and `contracts` profiles mechanically instead of hand-writing envelopes.**
  - It creates the docblock when there is none and extends an existing one by inserting whole lines before the closing `*/` — every pre-existing line is preserved byte for byte — and only ever writes escapes the engine *proves*: a declaration whose escapes are all `Maybe` is refused with `escape-not-proven`, never annotated. Running it twice is a no-op (`already-declared`), and like its siblings it is dry-run by default with `--apply` gated on the zero-new-diagnostics post-check, which is measured on the default display surface. Seeding an override whose parent declares a narrower envelope can therefore make `throw.liskov-widened` visible on a `contracts` run — pre-existing debt the envelope exposes, not a new finding on the default surface.

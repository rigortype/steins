...
- **The effect vocabulary ships as its own package.** `rigortype/steins-attributes` provides `#[\Steins\Pure]` and `#[\Steins\Effect]` — the classes you write in your own source to declare an effect envelope. It is MIT and inert at runtime, separate from the Apache-2.0 analyzer because it is vocabulary rather than tooling and its spread is the point (ADR-0025). The Composer package requires it, so one command leaves you able to write an envelope; nothing changes for the Homebrew or release-binary channels, where the attributes were always yours to install.

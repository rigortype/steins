...
| The `unset` pseudo-type's positions | 0087 | The **vocabulary** (issue #395) and the **top-level semantics** (issue #396) have both landed: `unset` in a phpdoc union lowers to its own leaf instead of a class named `unset`, is non-shadowable, round-trips through the speller, contributes no value — `\DateTime\|unset` accepts exactly what `\DateTime` accepts — and a **top-level inline** `@var T\|unset $x` now seeds the name as possibly-unbound, reported by `phpdoc.maybe-undefined` (`Layer::Contract`, `Floor::Contracts`) on any read `isset`/`empty`/`??`/`??=`/an assignment/the defaulting idiom has not discharged, over ADR-0081's own presence engine. What is left is **positions**: `@param`, `@return`, a property `@var` and a function-scope inline `@var` carry the corrected vocabulary and no semantics, deliberately undecided in ADR-0087 §5 — issue #397. Two residues of the landed slice (ADR-0087 §8): a nested `unset` (`array<int, unset>`) is one of those undecided positions and seeds nothing, and where two docblocks declare the same name the read attributes to the later one. |

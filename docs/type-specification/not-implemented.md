...
whole of what the alias table is applied to. Four positions therefore stay dark,
...
A body is read from **one physical line**: a declaration wrapped across lines is
not recovered, and the truncated half floors. And a **hyphenated** alias name is
refused whole rather than truncated at the hyphen (ADR-0091 §4.1;
divergence-registry entry 17).

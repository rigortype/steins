...
- **Positions the native relation does not model.** `array`, `iterable`,
  `callable`, `object` and a class-typed position all decline, so
  `array_map('f', 'notanarray')` and `str_replace([], 1, 1)` stay silent. Every
  one of those declines exactly as the same spelling written on a *project*
  parameter does — the cap is `NativeType`'s member set, in one place, for both.
  `resource` is the one exception, because the engine cannot spell it: an
  untyped position the pinned stub declares `@param resource` is judged from
  the consumer table (ADR-0097 §2.5), so `fwrite('x', …)` reports and
  `fwrite($notAResource, …)` reports when `$notAResource` is a proven value.
...
`@property`, `@property-read`, `@property-write`, `@mixin`. Steins recognizes
each tag's presence and its subject — the method name, the property name, the
`@mixin` target — and records one `(class-like, kind, subject)` obstacle per tag
site. A class-like carrying any of them anywhere in its resolved reach (parents,
interfaces, `@mixin` targets transitively) is not enumerable, so the absence
family is silent on it, exactly as for `__call`. Reading them as **member
sources** — resolving `$model->scopeActive()` or `$model->created_at` to a type
— remains deferred, as does the subject-granular discharge channel that would
re-enable the absence proof for a class-like's *undeclared* remainder
(ADR-0039's to design). Their types are never parsed: only the subject is. The
`@phpstan-type` / `@phpstan-import-type` pair left this set with issue #471 (it
declares no member) and stopped being unread with issue #472, below.

**Type aliases: the positions that stay dark** (issue #472). A
`@phpstan-type` / `@psalm-type` declaration on a **class-like** is in force over
that class-like's own member docblocks, and `@phpstan-import-type Name from
Other` resolves through `Other`'s table — the whole of #472's scope, and the
whole of what the alias table is applied to. Six positions therefore stay dark,
each a bounded gap rather than an oversight:

- **A free function's `@param`/`@return`.** The alias table is owned by a
  class-like and applied at the four sites that already apply the class-level
  `@template` shadow, all of which have the enclosing class-like in hand. A
  function declaration has none, and "which class-like encloses this offset"
  is an index question no seam answers today.
- **A closure's adopted docblock**, for the same reason the class-level
  `@template` shadow already stops there (issue #128's known limitation).
- **A property's `@var`**, which is a different lane: `parse_envelopes` reads
  `@param`/`@return`/assert tags, and the property lane resolves its own
  `@var` against the class shadow alone.
- **A second level of alias-names-alias, and any cycle**: one substitution at
  the use site plus one inside the body, then the floor — the "one level, no
  walk" bound ADR-0032's amendment set for `template-type`. An import of an
  import floors for the same reason.
- **The assertion lane.** `Envelopes::resolve_aliases` rewrites the parsed
  assert specs along with `@param`/`@return`, but the two consumers that read
  them (`asserts.rs`) call `Cx::envelopes_of` directly and run neither
  class-level stage — not the `@template` shadow either — so an alias in
  `@phpstan-assert Row $x` is not resolved. Silence, and the same silence the
  shadow's absence already produces there.
- **An alias used as a generic base** (`@param Row<int>`). The walk descends
  into a generic's *arguments*, not its base identifier, so the name reaches
  `lower_generic`'s class catch-all. PHPStan rejects the spelling outright; what
  Steins does with it is unchanged from before #472, and it is the one place the
  "an alias name never reaches the class machinery" invariant is not enforced
  structurally.

In every dark position the name lowers as it did before #472 — as a class, if
it names one, and otherwise through the unknown-name path. Inside the alias
machinery the floor is `Opaque`, never `ContractTy::Class`.

A body **is** reassembled across the physical lines it wraps: an alias tail has
no trailing anchor, so a wrap at `|`, `&` or inside an unclosed bracket leaves a
first line that is often a valid type on its own, and binding that prefix would
be a narrower contract than the author wrote — a manufactured `No`, which is the
one outcome #472 exists to prevent. What ends the body is what upstream also
leaves out of the type: a blank gutter line, a new `@tag`, the `*/` closer, and
prose. A tail ending in a dangling `|` or `&` floors rather than being completed
across the wrap, which is where upstream declares the tag invalid.

A **hyphenated** alias name is refused whole rather than truncated at the hyphen
(ADR-0091 §4.1; divergence-registry entry 17), and the refusal is **reported** —
`phpdoc.unknown-vocabulary` fires on the declaration line, not only where the
name is later used.

A name the vocabulary already owns binds nothing: `@phpstan-type integer …`
leaves `@param integer` reading `int`. The predicate is
`steins_contract::is_type_vocabulary` (does this identifier reach the lowering
tables' class catch-all), **not** `is_shadowable_pseudo_type` — a class may
shadow `integer`, an alias declaration may not rebind it, and taking the

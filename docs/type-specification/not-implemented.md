...
| `template-type<Subject, Owner, 'TName'>` resolution | 0032 | **Declared, unenforced.** The spelling is recognized vocabulary and lowers to `Opaque` at every arity (issue #360), so it constrains nothing and its owner argument is exempt from `untyped.generics` — a class-reference position, not a missing type argument. Reading the named `@template` out of the subject (the declared side, the receiver's carry, and a function-level template bound from an argument's carry) is issue #361 and its siblings under #359. |

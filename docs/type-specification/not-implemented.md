...
| The `unset` pseudo-type's semantics | 0087 | The **vocabulary has landed** (issue #395): `unset` in a phpdoc union lowers to its own leaf instead of a class named `unset`, is non-shadowable, round-trips through the speller, and contributes no value — `\DateTime\|unset` accepts exactly what `\DateTime` accepts. What has no code is what the state *means*: a top-level inline `@var T\|unset $x` stating that reads of `$x` may find no binding, `isset`/`empty`/`??`/`??=`/assignment as its discharge, the guard never reported redundant, and the `phpdoc.maybe-undefined` id (`Layer::Contract`, `Floor::Contracts`) that would carry it — issue #396. Positions other than an inline top-level `@var` (`@param`, `@return`, property `@var`, function-scope `@var`) carry the corrected vocabulary and no semantics, deliberately undecided in ADR-0087 §5 — issue #397. |

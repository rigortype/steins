...
走る — `list{…}` 受理の順列拒否を含む。ただし**綴り**は意図的に乖離では
ない: sealed shape は PHPStan と同一の綴りを出す(連続する必須キーは位置
記法 `array{…}`、それ以外は全キーを明示、必須キーが含意する `non-empty-`
は落とし、任意キーが 2 つ以上あって初めて意味を持つ `list` 語のみ残す)。
理由付きの

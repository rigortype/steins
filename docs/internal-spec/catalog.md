...
name to a **value-domain refinement string** — eleven rows today:
...
- **Builtin *signatures*.** The catalog carries effects, hierarchy, throws,
  failure arms, invocation shapes, and the curated **return-fact refinements** —
  but not parameter types or full return *types* (the reflected return envelope
  is the sidecar's `reflect`, not a catalog table). That is why

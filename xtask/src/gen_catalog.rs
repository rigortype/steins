...
fn render_declared_returns(
    ...
    let _ = writeln!(s, "//   {:>5}    RICHER than a single-base envelope (the issue-#79 reach)", counts.admitted_rich);
    s.push_str(
        "//\n\
         // The skipped methods and the two buckets above them are what remains deferred\n\
         // (ADR-0069 §5 as amended 2026-08-01). The array vocabulary lowers, but seeding\n\
         // it needs the shape lane and a shape-aware countersign; object arms have no\n\
         // scalar denotation, so the countersign could only answer `Maybe`.\n\
         //\n\
         ...
         // `T|false` failure union, or a refinement). The consumer re-lowers the string\n\

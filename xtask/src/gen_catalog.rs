...
//! declared-return envelope floor ([`gen_declared_envelopes`], from
//! `phpstan-mining/declared_envelopes.toml`, whose own source of record is produced
...
pub fn run() -> Result<(), String> {
    ...
    gen_return_facts()?;
    gen_declared_envelopes()?;
    Ok(())
}

/// Regenerate the builtin declared-return **envelope floor** (ADR-0069, issue #73)
/// from `docs/research/phpstan-mining/declared_envelopes.toml` into
/// `crates/steins-catalog/src/declared_envelopes_generated.rs`.
///
/// Two tables come out of one source of record: the envelope rows themselves, and
...
fn gen_declared_envelopes() -> Result<(), String> {
    let src = repo_root().join("docs/research/phpstan-mining/declared_envelopes.toml");
    let text = std::fs::read_to_string(&src).map_err(|e| format!("read {}: {e}", src.display()))?;
    ...
    let out = render_declared_envelopes(&doc.meta, &doc.counts, &rows, &sensitive);
    let dst = repo_root().join("crates/steins-catalog/src/declared_envelopes_generated.rs");
    std::fs::write(&dst, &out).map_err(|e| format!("write {}: {e}", dst.display()))?;
    println!(
        "gen-catalog: {} declared-envelope rows + {} version-sensitive names emitted → {}",
        rows.len(),
        sensitive.len(),
        dst.display()
    );
...
/// The shape of `declared_envelopes.toml`. Exclusion sections are documentation of
...
struct EnvelopeCounts {
    ...
    not_lowerable_multi_base_unions: usize,
    reflection_disagree: usize,
    reflection_missing: usize,
    admitted: usize,
}

/// Render the committed declared-envelope tables, provenance header and all.
fn render_declared_envelopes(
    meta: &EnvelopeMeta,
    counts: &EnvelopeCounts,
    ...
    let _ = writeln!(s, "//   {:>5}  rows richer than an envelope, of which", counts.not_lowerable);
    let _ = writeln!(s, "//   {:>5}    shaped arrays / lists", counts.not_lowerable_shaped_arrays);
    let _ = writeln!(s, "//   {:>5}    multi-base unions (the `T|false` family)", counts.not_lowerable_multi_base_unions);
    let _ = writeln!(s, "//   {:>5}  rows the pinned engine's own declaration escapes", counts.reflection_disagree);
    let _ = writeln!(s, "//   {:>5}  names the pinned engine does not know as functions", counts.reflection_missing);
    let _ = writeln!(s, "//   {:>5}  ADMITTED (the table below)", counts.admitted);
    s.push_str(
        "//\n\
    ...
    s.push_str("pub(crate) static DECLARED_ENVELOPES: &[(&str, &str)] = &[\n");
    for (key, ty) in rows {
    ...
    s.push_str("pub(crate) static ENVELOPE_VERSION_SENSITIVE: &[(&str, (u16, u16))] = &[\n");

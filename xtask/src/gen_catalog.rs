...
//!
//! Four further tables ride the pipeline: curated return-fact refinements
//! ([`gen_return_facts`], `return_facts.toml`), the ADR-0069 declared-return
...
//! issue #673), both sourced by `cargo xtask mine-function-map` — and the
//! ADR-0097 §2.6 migrated-class table ([`gen_migrated_resource_classes`],
//! `phpstan-mining/migrated_resource_classes.toml`, sourced by
//! `mine-function-map --migrated` at the declared-return table's own pin).
//!
...
pub fn run(check: bool) -> Result<(), String> {
    ...
    gen_resource_params(check)?;
    gen_declared_returns(check)?;
    gen_declared_method_returns(check)?;
    gen_migrated_resource_classes(check)?;
    gen_param_facts(check)?;
    gen_constants(check)?;
    Ok(())
}

/// Regenerate the **migrated-class** table (ADR-0097 §2.6) from
/// `phpstan-mining/migrated_resource_classes.toml` into
/// `migrated_resource_classes_generated.rs`. One field survives transcription —
/// the class name, keyed lowercase for the lookup with the engine's own casing
/// beside it; the `from` lists and the `[declined]` buckets are the audit trail
/// and stay in the source of record. Sourced by
/// `cargo xtask mine-function-map --migrated`.
fn gen_migrated_resource_classes(check: bool) -> Result<(), String> {
    let src = repo_root().join("docs/research/phpstan-mining/migrated_resource_classes.toml");
    let text = std::fs::read_to_string(&src).map_err(|e| format!("read {}: {e}", src.display()))?;
    let doc: MigratedDoc = toml::from_str(&text).map_err(|e| format!("parse {}: {e}", src.display()))?;

    let mut table: BTreeMap<String, String> = BTreeMap::new();
    for c in &doc.class {
        let name = c.name.trim_start_matches('\\');
        if name.is_empty() || name.contains('-') {
            return Err(format!("migrated-class row `{}` is not a class name", c.name));
        }
        if c.from.is_empty() {
            return Err(format!("migrated-class row `{name}` names no function that declares it"));
        }
        if let Some(prev) = table.insert(name.to_ascii_lowercase(), name.to_owned()) {
            return Err(format!("duplicate migrated-class row: `{prev}` and `{name}`"));
        }
    }
    if table.len() != doc.counts.classes {
        return Err(format!(
            "migrated_resource_classes.toml says {} classes and lists {}",
            doc.counts.classes,
            table.len()
        ));
    }

    let out = render_migrated_resource_classes(&doc.meta, &doc.counts, &table);
    let dst = repo_root().join("crates/steins-catalog/src/migrated_resource_classes_generated.rs");
    emit(&dst, &out, check)?;
    println!("gen-catalog: {} migrated-class rows {} → {}", table.len(), verb(check), dst.display());
    Ok(())
}

/// The shape of `migrated_resource_classes.toml`. `[migrated]` and `[declined]`
/// document the derivation and are deliberately not read — nothing is generated
/// from them.
...
/// Render the committed migrated-class table. Deterministic (BTreeMap order).
fn render_migrated_resource_classes(
    meta: &MigratedMeta,
    counts: &MigratedCounts,
    ...
    let _ = writeln!(s, "// phpstan-src pin: {} (declared_returns.toml's, read with `git show`)", meta.phpstan_src_commit);
    let _ = writeln!(s, "// asked of PHP {} via the real sidecar.", meta.php);
    s.push_str("//\n// Counts at the pin:\n");
    let _ = writeln!(s, "//   {:>5}  base-map functions whose return type spells `resource`", counts.resource_rows);
    let _ = writeln!(s, "//   {:>5}  of those, the engine declares a class for (the `from` lists in the TOML)", counts.migrated_functions);
    let _ = writeln!(s, "//   {:>5}  distinct classes (the table below)", counts.classes);
    let _ = writeln!(s, "//   {:>5}  functions contributing no class, of which", counts.declined);
    let _ = writeln!(s, "//   {:>5}    the engine does not have (an unloaded extension)", counts.declined_unloaded);
    let _ = writeln!(s, "//   {:>5}    still declare no return type (the producers ADR-0056 §8.2 admits)", counts.declined_typeless);
    let _ = writeln!(s, "//   {:>5}    declare a type naming no class", counts.declined_no_class);
    s.push_str(
        "//\n\
    ...
    for (key, name) in table {
        let _ = writeln!(s, "    ({key:?}, {name:?}),");
    }
    s.push_str("];\n");
    s
}

/// Regenerate the **resource-return** table (ADR-0056 §8, ADR-0097 §2.1) from
/// `resource_returns.toml` into `resource_returns_generated.rs`. Three fields
/// survive transcription — name, whether the stub's `@return` carries a `false`
/// arm, and the handle's kind; the rest (stub path, probe transcript, confidence
...
fn gen_resource_returns(check: bool) -> Result<(), String> {
    ...
    let mut table: BTreeMap<String, (bool, &'static str)> = BTreeMap::new();
    for f in &doc.function {
        ...
        // The kind vocabulary is `steins_catalog::ResourceKind`'s, spelled as the
        // TOML spells it; a row naming any other kind must fail the build rather
        // than land as a variant the closing table has never probed.
        let kind = match f.kind.as_str() {
            "stream" => "Stream",
            "persistent-stream" => "PersistentStream",
            "dir" => "Dir",
            "process" => "Process",
            "stream-context" => "StreamContext",
            "stream-filter" => "StreamFilter",
            other => {
                return Err(format!("resource-return row `{key}`: unexpected kind `{other}`"));
            }
        };
        if table.insert(key.clone(), (may_be_false, kind)).is_some() {
...
fn render_resource_returns(table: &BTreeMap<String, (bool, &'static str)>) -> String {
    let mut s = String::new();
    s.push_str(
        "// @generated by `cargo xtask gen-catalog` from\n\
    ...
    s.push_str("pub(crate) static RESOURCE_RETURNS: &[(&str, bool, ResourceKind)] = &[\n");
    for (key, (may_be_false, kind)) in table {
        s.push_str(&format!("    ({key:?}, {may_be_false}, ResourceKind::{kind}),\n"));
    }
    s.push_str("];\n");
    s
}

/// Regenerate the **resource-parameter** table (ADR-0097 §2.5, the consumer
/// twin of [`gen_resource_returns`]) from `resource_params.toml`, produced by
/// `cargo xtask mine-resource-params`, into `resource_params_generated.rs`.
/// Five fields survive transcription — function, position, the parameter's
/// name, and the two curated bits with the optional kind; the stub path and
/// the probe transcript are evidence that belongs in the source of record.
fn gen_resource_params(check: bool) -> Result<(), String> {
    let src = repo_root().join("docs/research/phpsrc-mining/resource_params.toml");
    let text = std::fs::read_to_string(&src).map_err(|e| format!("read {}: {e}", src.display()))?;
    let doc: ResourceParamDoc =
        toml::from_str(&text).map_err(|e| format!("parse {}: {e}", src.display()))?;

    let mut table: BTreeMap<(String, usize), ResourceParamRow> = BTreeMap::new();
    for p in &doc.param {
        let key = (p.function.to_ascii_lowercase(), p.index);
        if p.kind.is_some() && p.probe.is_none() {
            return Err(format!(
                "resource-param row `{}` #{}: a `kind` with no `probe` — a kind is a measured claim",
                key.0, key.1
            ));
        }
        if let Some(kind) = &p.kind
            && !["stream", "dir", "process", "stream-context", "stream-filter"].contains(&kind.as_str())
        {
            return Err(format!("resource-param row `{}` #{}: unknown kind `{kind}`", key.0, key.1));
        }
        if table.insert(key.clone(), p.clone()).is_some() {
            return Err(format!("duplicate resource-param row for `{}` #{}", key.0, key.1));
        }
    }

    let out = render_resource_params(&doc.meta, &table);
    let dst = repo_root().join("crates/steins-catalog/src/resource_params_generated.rs");
    emit(&dst, &out, check)?;
    println!("gen-catalog: {} resource-param rows {} → {}", table.len(), verb(check), dst.display());
    Ok(())
}

/// The shape of `resource_params.toml`; `stub` and `probe` are evidence and, past
/// the kind-needs-a-probe check, ignored.
#[derive(serde::Deserialize)]
struct ResourceParamDoc {
    meta: ResourceParamMeta,
    #[serde(default)]
    param: Vec<ResourceParamRow>,
}

#[derive(serde::Deserialize)]
struct ResourceParamMeta {
    php_src_commit: String,
...
/// Render the committed resource-parameter table. Deterministic (BTreeMap order,
/// `(function, index)` ascending for binary search).
fn render_resource_params(
    meta: &ResourceParamMeta,
    table: &BTreeMap<(String, usize), ResourceParamRow>,
) -> String {
    use std::fmt::Write as _;
    let mut s = String::new();
    s.push_str(
    ...
    let _ = writeln!(s, "// php-src pin {}; probes at PHP {}.", meta.php_src_commit, meta.probe_php.as_deref().unwrap_or("(none)"));
    s.push_str("//\n// Counts at the mining pin:\n");
    let _ = writeln!(s, "//   {:>5}  rows (one per exactly-`resource` position on a free function)", meta.rows);
    let _ = writeln!(s, "//   {:>5}  positions declined — unions, by-ref, variadic, natively typed, null default", meta.declined);
    let _ = writeln!(s, "//   {:>5}  method positions skipped (ADR-0056 §4's function-keyed bound)", meta.methods_skipped);
    let _ = writeln!(s, "//   {:>5}  rows carrying a probe transcript", meta.probed);
    s.push_str(
        "\n/// What one builtin position demands of a resource argument (ADR-0097 §2.5).\n\
         ...
         \x20   /// The parameter's declared name without the `$`, so a finding can name\n\
         ...
         \x20   pub closes: bool,\n\
         \x20   /// The resource kind the position demands, where a probe established it:\n\
         \x20   /// `stream`, `dir`, `process`, `stream-context`, `stream-filter`.\n\
         \x20   pub kind: Option<&'static str>,\n\
         }\n\n",
    );
    let _ = writeln!(s, "/// Sorted by `(function, index)` for binary search; functions are lowercase.");
    let _ = writeln!(s, "pub(crate) static RESOURCE_PARAMS: &[(&str, usize, ResourceParam)] = &[");
    for ((function, index), r) in table {
        let _ = writeln!(
            s,
            "    ({function:?}, {index}, ResourceParam {{ name: {:?}, accepts_closed: {}, closes: {}, kind: {:?} }}),",
            r.name, r.accepts_closed, r.closes, r.kind
        );
    }
    let _ = writeln!(s, "];");
    s
...
struct ParamMeta {
    ...
    /// `["8.5.10", "Darwin"]` per build the union asked, in the order asked
    /// (issue #703). Empty on a table mined before the slice.
    #[serde(default)]
    engines: Vec<Vec<String>>,
}
...
fn render_param_facts(
    ...
    s.push_str(
        "// @generated by `cargo xtask gen-catalog` from\n\
         ...
         //\n\
         // Builtin PER-PARAMETER FACTS (issue #382), read off the engines' own arginfo\n\
    ...
    if meta.engines.len() > 1 {
        // A build is one operating system, and `chroot` is a Linux builtin no
        // Darwin PHP has at any minor — so the rows are the union across these,
        // and a name the sources disagree about by-ref is refused rather than
        // merged (issue #703).
        s.push_str("//\n// Every build the union asked, in the order asked:\n");
        for e in &meta.engines {
            let (php, os) = (e.first().map_or("", String::as_str), e.get(1).map_or("", String::as_str));
            let _ = writeln!(s, "//   PHP {php}  {os}");
        }
    }
    s.push_str(

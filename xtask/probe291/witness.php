<?php
// issue #291 probe — the coercion witness grid, run on the pinned PHP.
//
// Usage: php witness.php strict   /  php witness.php coercive
//
...
// The witness printed is the value's literal spelling, so the table's rows are
// reproducible one-liners.

$mode = $argv[1] ?? 'coercive';
$dir = sys_get_temp_dir() . '/steins-probe291';
@mkdir($dir);
...
// One witness per equivalence class of PHP's parameter-coercion behaviour.
$values = [
    'int'                 => '0',
    'float(1.5)'          => '1.5',
    'float(1.0)'          => '1.0',
